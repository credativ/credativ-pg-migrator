...
- 2026.07.29
  ...
  - Feature - Connectors / Base Database Connector: Added automatic conversion of MySQL/MariaDB `FIND_IN_SET(str, strlist)` to PostgreSQL native array functions (`coalesce(array_position(string_to_array(strlist, ','), str), 0)`), preventing `function find_in_set(unknown, text) does not exist` errors in PostgreSQL view and query execution. Also added automatic conversion of `GROUP BY <exprs> WITH ROLLUP` to ANSI/PostgreSQL standard `GROUP BY ROLLUP (<exprs>)`, stripping of character set and collation specifications (`CHARACTER SET <charset>`, `CHARSET <charset>`, `COLLATE <collation>`), character set introducers (`_utf8mb4'...'`, `_latin1"..."`), and backslash quote escapes (`\'`, `\"`) when converting SQL code, views, functions, and expression indexes for target PostgreSQL. Transpiles MySQL multi-valued/JSON index expressions (`CAST(... AS CHAR(32) ARRAY)`) to valid PostgreSQL dialect (`CAST(... AS CHAR(32)[])`).
