...
class MigratorTables:
    ...
    def decode_sequence_row(self, row):
        return {
            ...
            'source_last_value': row[12],
            'source_next_value': row[13],
            'source_sequence_comment': row[14],
            'target_schema_name': row[15],
            'target_table_name': row[16],
            'target_column_name': row[17],
            'target_sequence_name': row[18],
            'target_sequence_sql': row[19],
            'target_sequence_comment': row[20]
        }
    ...
    def create_table_for_validation(self):
        ...
        create_columns_query = f"""
            CREATE TABLE IF NOT EXISTS "{self.protocol_schema}"."{self.config_parser.get_validation_columns_name()}" (
                ...
                target_null_count bigint,
                source_empty_string_count bigint,
                target_empty_string_count bigint,
