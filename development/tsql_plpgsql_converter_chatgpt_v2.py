...
def _convert_if_statements(sql: str) -> str:
    ...
    for line in lines:
        ...
        if lower.startswith('else if') or lower.startswith('elseif'):
            ...
            # Insert THEN before any trailing comment
            if ' then' not in rest.lower():
                rest_no_semicolon = rest.rstrip(';')
                # Split on inline comment marker
                comment_pos = rest_no_semicolon.lower().find('--')
                if comment_pos != -1:
                    before_comment = rest_no_semicolon[:comment_pos].rstrip()
                    comment = rest_no_semicolon[comment_pos:]
                    rest = f"{before_comment} THEN {comment}"
                else:
                    rest = rest_no_semicolon + ' THEN'
...
# New helper to close IF blocks using END IF; terminators.
def _close_if_blocks(sql: str) -> str:
    """Ensure that IF/ELSIF blocks have corresponding END IF; statements.

    This helper traverses the lines of SQL and uses a simple stack to
    differentiate between control flow blocks opened by ``IF/ELSIF`` and
    generic ``BEGIN``/``END`` pairs.  For each ``IF`` or ``ELSIF`` a
    marker ``'IF'`` is pushed onto the stack.  For every ``BEGIN`` a
    ``'BEGIN'`` marker is pushed.  When an ``END`` is encountered the
    top of the stack determines how it is closed:

    * If the marker is ``'BEGIN'`` the ``END`` is assumed to close a
      generic block and is removed entirely (PL/pgSQL does not require
      explicit ``BEGIN``/``END`` delimiters inside IF branches).
    * If the marker is ``'IF'`` an ``END IF;`` line is emitted.
    * If the stack is empty the ``END`` closes the procedure/function
      body and results in ``END;``.

    At the end of the scan any remaining ``'IF'`` markers are closed with
    ``END IF;``.  This heuristic avoids producing spurious ``END IF``
    terminators for nested ``BEGIN … END`` blocks that are not associated
    with control flow.
    """
    lines = sql.splitlines()
    stack: List[str] = []
    ...
    while i < n:
        ...
        # Handle ELSIF separately: part of an existing IF chain, do not push
        if upper.startswith('ELSIF '):
            out_lines.append(line)
            i += 1
            continue
        # Push IF markers for new IF control structures
        if upper.startswith('IF ') or upper.startswith('IF('):
            stack.append('IF')
            out_lines.append(line)
            i += 1
            continue
        # Push BEGIN markers to balance ENDs.  Keep the BEGIN line for now;
        # later passes may remove BEGIN/END wrappers.
        if upper.startswith('BEGIN'):
            stack.append('BEGIN')
            out_lines.append(line)
            i += 1
            continue
        # Handle END statements that are not already ``END IF``
        if upper.startswith('END') and not upper.startswith('END IF'):
            if stack:
                marker = stack.pop()
                if marker == 'BEGIN':
                    # END corresponding to a BEGIN; drop it entirely
                    # If the next non‑empty line does not begin with ELSE/ELSIF then
                    # close the preceding IF (if present on the stack).  This
                    # handles patterns like ``IF … BEGIN … END`` without
                    # ELSE clauses by emitting ``END IF;`` immediately.
                    ...
                    while j < n:
                        ...
                        if next_stripped == '' or next_stripped.startswith('--'):
                            j += 1
                            continue
                        peek_upper = next_stripped.upper()
                        break
                    if stack and stack[-1] == 'IF':
                        if not (peek_upper.startswith('ELSE') or peek_upper.startswith('ELSIF')):
                            stack.pop()
                            out_lines.append('END IF;')
                    i += 1
                    continue
                elif marker == 'IF':
                    # Close an IF/ELSIF block
                    out_lines.append('END IF;')
                    i += 1
                    continue
            # If stack is empty or marker not handled emit a generic END;
            # this typically closes the procedure/function body.
    ...
    # Only close IF markers; drop BEGIN markers silently
    while stack:
        marker = stack.pop()
        if marker == 'IF':
            out_lines.append('END IF;')
...
def _sanitize_final(sql: str) -> str:
    ...
    # Append a semicolon to lone END statements (e.g. END;) but avoid
    # changing control flow constructs such as END IF/LOOP/WHILE/CASE.  In
    # T‑SQL these blocks are closed with just END but in PL/pgSQL the
    # corresponding control statement (IF, LOOP, WHILE, CASE) requires an
    # ``END IF;`` terminator.  We handle those explicitly above and only
    # add a semicolon to ``END`` when it is not followed by one of these
    # keywords or an existing semicolon.  Without this check the naïve
    # pattern would convert ``END IF;`` into ``END; IF;`` which breaks
    # nested IF/ELSE chains.
    sql = re.sub(r'\bEND\b(?!\s+(?:IF|LOOP|WHILE|CASE)\b)(?!;)', 'END;', sql, flags=re.IGNORECASE)
...
def convert_heuristic(tsql: str) -> Tuple[str, str]:
    ...
    body_term = _convert_print_and_raiserror(body_term)
    # Close IF blocks by converting END statements into END IF; where appropriate
    body_term = _close_if_blocks(body_term)
