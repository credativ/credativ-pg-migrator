...
class MariaDBConnector(DatabaseConnector):
    ...
    def fetch_table_names(self, table_schema: str):
        ...
        try:
            ...
            self.config_parser.print_log_message('ERROR', f"mariadb_connector: fetch_table_names: Error fetching table names: {e}")
            raise
    ...
    def fetch_table_columns(self, settings) -> dict:
        ...
        try:
            ...
            self.config_parser.print_log_message('ERROR', f"mariadb_connector: fetch_table_columns: Error fetching table columns: {e}")
            self.config_parser.print_log_message('ERROR', "mariadb_connector: fetch_table_columns: Full stack trace:")
    ...
    def migrate_table(self, migrate_target_connection, settings):
        ...
        try:
            ...
            if source_table_rows_limited == 0:
                self.config_parser.print_log_message('INFO', f"mariadb_connector: migrate_table: Worker {worker_id}: Table {source_table_name} is empty - skipping data migration.")
                ...
                if source_table_rows_limited > target_table_rows or data_conflict_action in ('merge_keep_target', 'merge_keep_source', 'replace'):
                    ...
                    self.config_parser.print_log_message('INFO', f"mariadb_connector: migrate_table: Worker {worker_id}: Source table {source_table_name}: {source_table_rows_limited} rows / Target table {target_table_name}: {target_table_rows} rows - starting data migration.")
                    ...
                    if resume_after_crash and not drop_unfinished_tables:
                        ...
                        self.config_parser.print_log_message('DEBUG', f"mariadb_connector: migrate_table: Worker {worker_id}: Resuming migration for table {source_schema_name}.{source_table_name} from chunk {chunk_number} with data chunk size {chunk_size}.")
                    ...
                    self.config_parser.print_log_message('DEBUG', f"mariadb_connector: migrate_table: Worker {worker_id}: Migrating table {source_schema_name}.{source_table_name}: chunk {chunk_number}, data chunk size {chunk_size}, batch size {batch_size}, chunk offset {chunk_offset}, chunk end row number {chunk_end_row_number}, source table rows {source_table_rows_limited}")
                    order_by_clause = ''
                    ...
                    self.config_parser.print_log_message('DEBUG2', f"mariadb_connector: migrate_table: Worker {worker_id}: Primary key columns for {source_schema_name}.{source_table_name}: {primary_key_columns}")
                    ...
                    self.config_parser.print_log_message('DEBUG', f"mariadb_connector: migrate_table: Worker {worker_id}: Fetching data with cursor using query: {query}")
                    ...
                    while True:
                        ...
                        self.config_parser.print_log_message('DEBUG',f"mariadb_connector: migrate_table: Worker {worker_id}: Fetched {len(records)} rows (batch {batch_number}) from source table {source_table_name}.")
                        ...
                        self.config_parser.print_log_message('DEBUG',
                            f"Worker {worker_id}: Starting insert of {len(records)} rows from source table {source_table_name}")
                    ...
                    target_table_rows = migrate_target_connection.get_rows_count(target_schema_name, target_table_name)
                    self.config_parser.print_log_message('INFO', f"mariadb_connector: migrate_table: Worker {worker_id}: Target table {target_schema_name}.{target_table_name} has {target_table_rows} rows")
                    ...
                    self.config_parser.print_log_message('INFO', f"mariadb_connector: migrate_table: Worker {worker_id}: Migrated {total_inserted_rows} rows from {source_table_name} to {target_schema_name}.{target_table_name} in {batch_number} batches: "
                    ...
                    self.config_parser.print_log_message('INFO', f"mariadb_connector: migrate_table: Worker {worker_id}: Source table {source_table_name} has {source_table_rows_limited} rows, which is less than or equal to target table {target_table_name} with {target_table_rows} rows. No data migration needed.")
                ...
                self.config_parser.print_log_message('DEBUG', f"mariadb_connector: migrate_table: Worker {worker_id}: Migration stats: {migration_stats}")
                if source_table_rows_limited <= target_table_rows or chunk_number >= total_chunks:
                    self.config_parser.print_log_message('DEBUG3', f"mariadb_connector: migrate_table: Worker {worker_id}: Setting migration status to finished for table {source_table_name} (chunk {chunk_number}/{total_chunks})")
            ...
            self.config_parser.print_log_message('ERROR', f"mariadb_connector: migrate_table: Worker {worker_id}: Error during {part_name} -> {e}")
    ...
    def fetch_views_names(self, source_schema_name: str):
        ...
        try:
            ...
            self.config_parser.print_log_message('ERROR', f"mariadb_connector: fetch_views_names: Error fetching view names: {e}")
            raise
    ...
    def execute_query(self, query: str, params=None):
        try:
            ...
            self.config_parser.print_log_message('ERROR', f"mariadb_connector: execute_query: Error executing query: {e}")
            raise
    ...
    def execute_sql_script(self, script_path: str):
        try:
            ...
            self.config_parser.print_log_message('ERROR', f"mariadb_connector: execute_sql_script: Error executing SQL script: {e}")
            raise
    ...
    def get_rows_count(self, table_schema: str, table_name: str, migration_limitation: str = None):
        ...
        try:
            ...
            self.config_parser.print_log_message('ERROR', f"mariadb_connector: get_rows_count: Error fetching row count: {e}")
            raise
    ...
    def get_table_size(self, table_schema: str, table_name: str):
        ...
        try:
            ...
            self.config_parser.print_log_message('ERROR', f"mariadb_connector: get_table_size: Error fetching table size: {e}")
            raise
    ...
    def get_table_next_identity(self, table_schema: str, table_name: str):
        try:
            ...
            self.config_parser.print_log_message('WARNING', f"mariadb_connector: get_table_next_identity: Error fetching next identity for {table_schema}.{table_name}: {e}")
            return None
    ...
    def get_table_description(self, settings) -> dict:
        self.config_parser.print_log_message('DEBUG3', f"mariadb_connector: get_table_description: MariaDB connector: Getting table description for {settings['table_schema']}.{settings['table_name']}")
        table_schema = settings['table_schema']
        table_name = settings['table_name']
        output = ""
        try:
            ...
            self.config_parser.print_log_message('ERROR', f"mariadb_connector: get_table_description: Error fetching table description for {table_schema}.{table_name}: {e}")
            raise
    ...
    def get_database_size(self):
        ...
        try:
            ...
            self.config_parser.print_log_message('ERROR', f"mariadb_connector: get_database_size: Error fetching database size: {e}")
            raise
        ...
        try:
            ...
            if top_n > 0:
                query = f"""
                    ...
                    FROM information_schema.tables
                    WHERE TABLE_SCHEMA = '{settings['source_schema_name']}'
                ...
                cursor.close()
                self.disconnect()
                self.config_parser.print_log_message('DEBUG2', f"mariadb_connector: get_top_n_tables: Top {top_n} tables by rows: {top_tables['by_rows']}")
            else:
                self.config_parser.print_log_message('DEBUG', "mariadb_connector: get_top_n_tables: Top N tables by rows is not configured or set to 0, skipping this part.")

        except Exception as e:
            self.config_parser.print_log_message('ERROR', f"mariadb_connector: get_top_n_tables: Error fetching top {top_n} tables by rows: {e}")
    ...
    def target_table_exists(self, target_schema_name, target_table_name):
        ...
        try:
            ...
            self.config_parser.print_log_message('ERROR', f"mariadb_connector: target_table_exists: Error checking if target table exists: {e}")
            raise
