...
- 2026.07.22
  ...
  - Feature - Oracle Connector: Migrate views and materialized views with real Oracle→PostgreSQL query conversion. `fetch_views_names` now returns materialized views (`ALL_MVIEWS`) alongside regular views tagged with `view_type`, `fetch_view_code` falls back to `ALL_MVIEWS.QUERY`, and `convert_view_code` transpiles the defining query with `sqlglot` (read=`oracle`, write=`postgres`) and wraps it into a full `CREATE [MATERIALIZED] VIEW` statement (this also fixes regular Oracle views, which previously produced a bare, non-executable SELECT). Post-transpile fixes handle `SYSTIMESTAMP`→`CURRENT_TIMESTAMP`, `seq.NEXTVAL`→`nextval('seq')`, `FROM dual` removal and common `LISTAGG`→`STRING_AGG`; constructs that cannot be auto-converted (`(+)` outer joins, `CONNECT BY`/`START WITH`, `ROWNUM`, complex `LISTAGG`) are logged as `WARNING`s for manual review. Materialized view container tables are excluded from `fetch_table_names` so they are not additionally migrated as base tables.
