...
| Partitioning of source tables[16]         | yes     | yes     | yes     | yes      | --     | --    | --      | yes    | yes        | N/A      | N/A    | --     |
...
- [16]: Whether the connector reads how the SOURCE partitions its tables, so that the run can say what is there before it decides anything - and, with `migration.source_partitioning: preserve` (the default), build the same scheme on the target. PostgreSQL, Oracle, the three Db2 connectors and Informix read it. For Db2 the trap is the bound itself: Db2 writes `ENDING AT (x) INCLUSIVE` and means that x is in the partition, PostgreSQL's `TO (b)` means b is not, and **INCLUSIVE is Db2's default** - so a scheme copied bound for bound gives a target which refuses every row of the last day of every partition. Each inclusive end is carried over as the exclusive bound holding the same rows (`ENDING AT '2024-12-31'` becomes `TO ('2025-01-01')`), and where the column type has no next value - a DECIMAL with a scale, a VARCHAR, a TIMESTAMP - the scheme is refused rather than moved by a guess. Two of the three things Db2 calls partitioning are not table partitioning at all and are reported per table rather than carried over: **DPF** spreads the rows over the physical nodes of the instance and has no counterpart of any kind, and **MDC** is a storage layout whose nearest PostgreSQL relatives are `CLUSTER` and BRIN. z/OS **partition-by-growth** has no partitioning key - a partition exists because the one before it filled up - and stops the run instead of arriving as something else. Db2 for LUW read through `system_catalog: SYSIBM` reports the scheme as not read, because the standard views describe none of the three. For **Informix** the answer is mostly the report itself: a table is *fragmented* across dbspaces to spread its I/O over devices, so the fragment is a place and not a class of row, and PostgreSQL does not need a partition to put a table in a tablespace. `sysfragments` is read in full - the strategy, the fragment expressions, the dbspaces, and the rows per fragment, so that a scheme where 95% of the rows sit in one fragment is seen before it is reproduced - and a PostgreSQL scheme is built only where the fragments really are a range or a list over one column. **ROUND ROBIN** has no key of any kind and **HYBRID** has two levels; both stop the run. An arbitrary boolean expression is a rewrite and not a translation and is refused with its text quoted. The one thing which makes an expression scheme readable is that Informix stops at the first fragment whose expression is true, so a chain of `col < v` is a chain of contiguous ranges rather than the overlapping set it reads as literally, and `REMAINDER` becomes the DEFAULT partition. For Oracle the first level is carried over and a sub-partitioned second level is **not**: a monthly range sub-partitioned by hash into 16 is 960 relations on the target, and the run reports how many segments were left behind rather than building them. Oracle's INTERVAL scheme keeps the partitions which exist and the run says that the automatic extension stops; REFERENCE and SYSTEM partitioning have no partitioning key to migrate at all and stop the run instead of arriving as something else. SQL Anywhere and SQLite have no partitioning, which is an absence of the engine and not of this migrator. Every other connector declares in `OBJECT_KINDS_NOT_READ` what its engine partitions and which catalogue keeps it, so a partitioned table arriving as one ordinary table is reported rather than silent - see `development/PARTITIONING_STRATEGY.md` §2.4 for the order the rest are built in.
- [17]: Creating the target table partitioned, from the `target_partitioning` block of the configuration, whether or not the source table was partitioned. It reads a configuration and writes PostgreSQL and nothing in it looks at what the source was, so it works for every connector: the target scheme is an input and not a derivation. The one thing a connector owes it is the smallest and the largest value of the partitioning column, asked in the quoting of the source, which the base connector provides for every connector with a connection. The two exceptions are **IBM Db2 for z/OS and IBM Db2 for i**, which are offline: the structure comes from `.sql` extracts and the data from CSV files, there is no instance to ask what a column holds, and an entry which generates its partitions from a `date_range` is therefore refused against those two sources before anything is created rather than producing a partitioned table with nothing under it. Before anything is created, every entry is checked against the table it really names - the columns exist and can carry the key, the primary key and every unique constraint and unique index contain the partitioning columns, the NULLs have somewhere to go, the partition count is sane and the generated names do not collide - and a check which could not be made is reported as not made rather than as one which passed.
