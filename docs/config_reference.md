...
The following top-level keys must be present: `exclude_tables`, `include_tables`, `migrator`, `source`, `target`.

include_tables and exclude_tables are required because config_parser reads them by direct indexing (config_parser.py:50 and :920) and raises KeyError when they are absent. Write [] when you do not want to filter. They become optional once those two lines use .get().
...
| `include_tables` | "all" \| list of string | `all` | `all` | **required**. Which tables to migrate. 'all' or an empty list means every table. A string other than 'all' is refused at startup. Write [] rather than leaving the key empty - an empty key parses as null. (read by direct indexing at config_parser.py:50) |
| `exclude_tables` | list of string \| null |  | `[]` | **required**. Names or patterns of tables to leave out, applied after include_tables. Write [] rather than leaving the key empty. (read by direct indexing at config_parser.py:920) |
| `include_views` | "all" \| list of string | `all` | `all` | Which views to migrate. 'all' or absent means every view. Note that an empty list matches nothing and silently skips every view. |
| `exclude_views` | list of string \| null |  | `[]` | Names or patterns of views to leave out, applied after include_views. |
| `include_funcprocs` | "all" \| list of string | `all` | `all` | Which functions and procedures to migrate. 'all' or absent means all of them. An empty list matches nothing and silently skips every routine. |
| `exclude_funcprocs` | list of string \| null |  | `[]` | Names or patterns of functions and procedures to leave out, applied after include_funcprocs. |
...
| `chunk_size` | integer |  | `-1` | Rows per independently logged and restartable chunk, for tables too large to copy in one pass. Must be larger than batch_size - a smaller value disables chunking with a warning. -1 disables chunking. Good values are 10x to 20x batch_size. |
| `names_case_handling` | string | `lower`, `upper`, `keep` | `keep` | Whether object names are lower-cased, upper-cased or kept as the source spells them. Validated at startup - any other value stops the run. |
