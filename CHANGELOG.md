...
- 2026.08.28
  ...
  - New - Documentation: **a page of what is not built yet** - [docs/todo.md](docs/todo.md), linked from the index and from §8.7 of the user guide, which listed two planned features and had been overtaken. It carries what remains open in the order it is worth doing, in the bands the project orders by: the decisions which shape the rest, what a run still cannot say about itself, the features which are half-built, and hygiene. An entry checked against the code is marked as such; the rest come from the design notes and say so, because some of them are older than the last repairs.

  - Fix - Documentation: **the page of the protocol tables described a schema the migrator does not create.** It named the tables of the mapping workflow `matching_tables` / `matching_columns`, which nothing is called, and **23 of the 53 tables** the migrator creates were not named on it at all - the whole validation set, the whole mapping set, the query conversion, the collations, the text search objects, the alterations of the target columns and the data sources among them. It is rewritten against `protocol_comments.py`, which is the catalogue the migrator writes into the database as `COMMENT ON`, so the page and what `\d+` answers now say the same thing. Two links of the user guide which pointed into a directory that does not exist are repaired with it.

  - New - Documentation: **the user guide says what decides the owner of the migrated objects.** `target.settings` had one sentence in the generated reference and nothing anywhere else. §5.3 now says that `role` is the owner of everything the migration creates, and the four things which decide whether that holds: an index, a constraint, a trigger and the sequence of an identity column follow the owner of their table; an object which already exists keeps its owner, so a second run over an older schema leaves a mixture; a session `SET` does not survive a connection pooler in transaction mode, which is how the ownership comes out mixed for no visible reason; and nothing verifies the result today - with the query which asks the catalogue instead.

  - New - Documentation: two behaviours which were in the code and in the release notes and in no manual. **The unsigned integers of Sybase ASE** (`usmallint` → `INTEGER`, `uint` → `BIGINT`, `ubigint` → `NUMERIC(20,0)`, the lower bound not carried over, and the identity column which is still forced to `BIGINT`), and **a table named with its database in front of it** (`ccd..t`, `ccd.dbo.t`) which Sybase ASE and MS SQL Server resolve without any configuration when the database is the one being migrated - including what is left as the source wrote it, and the two shapes which are not covered.
...
- 2026.08.26
  ...
  - Fix - **A foreign key knows which schema of the target it points at.** The `constraints` protocol table recorded the referenced table only as the source spells it, so everything downstream made its own guess: the `REFERENCES` clause was built with the schema of the REFERENCING table - the source schema was read into a variable which was then never used - and the orchestrator looked the referenced table up again by its source schema. Most sources report a foreign key inside one schema with an EMPTY referenced schema, and that lookup matched nothing, so such a constraint was skipped with `referenced table .ORDERS not found`, naming a schema nobody configured. The planner now decides where the referenced table lands and records it; a reference into a schema the run does not read is named as such, in the planning and again in the constraint which could not be created. A referenced table which is not part of the run is no longer a reason to skip the constraint - whether the target holds it is asked of the target.

  - Fix - The columns of the referenced table follow the convention of the rest of the protocol layer: `referenced_table_schema` / `referenced_table_name` are `source_referenced_table_schema` / `source_referenced_table_name`, and `target_referenced_table_schema` stands next to `target_referenced_table_name` - it did not exist at all. Part of P4-2 of `development/OPEN_ISSUES.md`.

  - Fix - The pass which aligns the column types of a foreign key looked the parent table up in a dictionary keyed by the name of the TARGET, using the name of the SOURCE - so with any `names_case_handling` but `keep` it found nothing and both sides silently kept the types of the source.

  - Fix - A constraint for which no statement could be built ended the worker with `UnboundLocalError` - it closed a connection which is opened in the branch above it - so the run reported an error of the migration instead of the constraint which was skipped. It is recorded as skipped now, with the reason. Two ways out of the same worker also left their connection to the target open, one per constraint which could not be created.

  - Fix - Sybase ASE: **a routine is created in the schema of the target.** The qualifier the source writes in its own header - `CREATE PROCEDURE dbo.acc2035c_getPermissionStatus` - was carried over as the schema of the generated routine, and only a header without one fell back to the target schema, so PostgreSQL answered `schema "dbo" does not exist` and the routine was not created at all. Everything else about the routine already used the target schema: the protocol records it there and the `COMMENT ON` is written for it. A routine written `database.owner.routine` also took its OWNER for its name; the name is the last part of it.

  - Fix - Transact-SQL routines (Sybase ASE and MS SQL Server): **a variable of the body is no longer written as a positional parameter.** Every parser of the family reads `@cid` as a parameter of the statement and writes it for PostgreSQL as `$cid`, so each statement of a routine which named a variable came back from the conversion with `$cid` in it - `UPDATE ... WHERE "customer_id" = $cid` - and the rename which turns `@cid` into `locvar_cid` no longer found it. The variables are carried through the conversion as the identifiers they become, and unquoted, so they are the ones the `DECLARE` block declares.

  - Fix - Transact-SQL routines: **`SELECT @a = 1, @b = 2` is an assignment, not a query.** It was handed to the statement converter, which made `SELECT $OptIn = 1, $OptOut = 2` of it - a statement of no dialect - and the pass which turns such a SELECT into the assignments of PL/pgSQL then no longer recognised it. The assignment is converted as the assignment it is, while its VALUES still go through the statement converter: the functions of the source, the string concatenation with `+` and the schema in front of a table are converted there as everywhere else, and an assignment with a `FROM` clause becomes the `SELECT ... INTO` of PL/pgSQL.

  - Fix - Transact-SQL routines: **an `@` inside a string literal is data.** The rename of the variables ran over the whole line, so a routine inserting `'mail to admin@example.com'` wrote `'mail to adminlocvar_example.com'` into the target - a routine which rewrites the text it inserts.
