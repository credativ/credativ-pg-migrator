...
- 2026.07.22
  ...
  - Feature - Oracle Connector: Migrate materialized views. `fetch_views_names` now returns materialized views (`ALL_MVIEWS`) alongside regular views tagged with `view_type`, `fetch_view_code` falls back to `ALL_MVIEWS.QUERY`, and `convert_view_code` wraps the definition into a full `CREATE [MATERIALIZED] VIEW` statement (this also fixes regular Oracle views, which previously produced a bare, non-executable SELECT). Materialized view container tables are excluded from `fetch_table_names` so they are not additionally migrated as base tables.
