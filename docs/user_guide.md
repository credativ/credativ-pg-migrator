...
- **Partitioning** has two halves which are worth telling apart. *Reading how the source partitions* is per connector and is implemented for PostgreSQL, Oracle and all three IBM Db2 connectors; every other connector declares what its engine partitions and which catalogue keeps it, so a partitioned table arriving as one ordinary table is reported rather than silent. Where an engine calls something "partitioning" which PostgreSQL has no counterpart for — Oracle's REFERENCE and SYSTEM schemes, Db2's DPF and MDC, z/OS's partition-by-growth — it is named per table rather than quietly dropped, and a scheme which cannot be built stops the run before anything is created. *Creating the target partitioned* — the `target_partitioning` block — reads a configuration and writes PostgreSQL, looks at nothing of the source, and therefore works for every connector. Both are reported in the `[ PARTITIONING ]` block of the pre-migration analysis before anything is created, and an entry which cannot be carried out stops the run there rather than in the middle of it.
...
Planned features:
- Reading the partitioning of the remaining sources: Informix, MS SQL Server, Sybase ASE and
