...
* Fixed IBM DB2 LUW function-based and XML index migration by joining `SYSCAT.INDEXES` with `SYSCAT.INDEXCOLUSE`, filtering out internal Db2 XML/block index types (`XPTH`, `XRGN`) and unindexable column types (`XML`, `CLOB`, `BLOB`, `DBCLOB`), and extracting expression definitions from `SYSCAT.INDEXCOLUSE.TEXT` with `is_function_based` set, resolving PostgreSQL index creation errors on Db2 internal virtual key columns (`column "K00" does not exist`) and duplicate XML index creation errors (`could not create unique index` on `SPEC_XML`)
