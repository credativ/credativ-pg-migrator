...
- 2026.07.27
  ...
  - Feature - Oracle Connector: Packages are migrated as standalone functions instead of being skipped. PostgreSQL has no packages, so the package body is split into its routines and each one is created as a function named `<package>_<routine>` (e.g. `PKG_AUDIT.LOG_CHANGE` → `pkg_audit_log_change`); a package procedure becomes a function returning `void`, or — when it has `OUT` parameters — a function whose result type PostgreSQL derives from them. Every call into a package is rewritten accordingly wherever this connector generates PL/pgSQL (functions, procedures and triggers), including unqualified calls between routines of the same package, and a call that forms a whole statement gets the `PERFORM` that PL/pgSQL requires — previously such a call was emitted as `pkg_audit.log_change(...);`, which is not valid PL/pgSQL and made every calling routine fail to create. The final object-validity pass accepts a package as valid when at least one of its functions exists.
