...
- Beyond the common set, some connectors reach further: PostgreSQL migrates partitioned tables, collations, full text search objects, domains and user-defined types; Sybase ASE migrates named default objects, rules/domains and user-defined types; Oracle migrates packages, object/collection types, 23ai domains and partitioned tables.
- **Partitioning** has two halves which are worth telling apart. *Reading how the source partitions* is per connector and is implemented for **every one of the twelve** whose engine has partitioning at all — PostgreSQL, Oracle, the three IBM Db2 connectors, Informix, MySQL/MariaDB, MS SQL Server and Sybase ASE (whose catalogue reading is written from documentation rather than against a live server, and which therefore reports what it could not read and refuses to build from it); SQL Anywhere and SQLite have none, and say so; every other connector declares what its engine partitions and which catalogue keeps it, so a partitioned table arriving as one ordinary table is reported rather than silent. Where an engine calls something "partitioning" which PostgreSQL has no counterpart for — Oracle's REFERENCE and SYSTEM schemes, Db2's DPF and MDC, z/OS's partition-by-growth, Informix's ROUND ROBIN fragmentation, MySQL's hash, which keeps its partition count and not the placement of a row, ASE's ROUND ROBIN, SQL Server's filegroups and its `RANGE LEFT` boundaries, which move by one value because PostgreSQL's upper bound is never inclusive — it is named per table rather than quietly dropped, and a scheme which cannot be built stops the run before anything is created. *Creating the target partitioned* — the `target_partitioning` block — reads a configuration and writes PostgreSQL, looks at nothing of the source, and therefore works for every connector. Both are reported in the `[ PARTITIONING ]` block of the pre-migration analysis before anything is created, and an entry which cannot be carried out stops the run there rather than in the middle of it.
...
Planned features:
- Confirming the Sybase ASE catalogue reading against a live server: it is written from the
  documentation of the engine, and the run says per table which of its reads answered.
- The `primary_key: extend | keep | drop` decision, and handing a generated scheme to
  `pg_partman` on the target — see `development/PARTITIONING_STRATEGY.md` §5.5 and §10.
- The `primary_key: extend | keep | drop` decision: today a key which does not contain the
  ...
  the repair.
- Handing the maintenance of a generated scheme to `pg_partman` on the target, instead of the
  fixed window of partitions the migration creates.
