...
- The conversion of application statements is implemented for **SAP SQL Anywhere**, and its conversion is a parse now instead of a chain of regular expressions: the `*=` outer join, the `TOP n START AT m` paging, `STRING()` and `IF … ENDIF` are rewritten before the statement is read as T-SQL, and the two things that parser means differently are corrected - `LOCATE(a, b)` searches the second argument in the first and T-SQL reads it the other way round, and `TIMESTAMP` is a date and a time there and the row version of a table in T-SQL. Six entries of its function mapping turned out to be defects: `CAST(a AS timestamp)` became `CAST(a AS CURRENT_TIMESTAMP)`, a table named `"user"` became `"CURRENT_USER"`, and `LOCATE`, `CHARINDEX` and `DATEFORMAT` were renamed into calls which were either invalid or answered another string. `NUMBER(*)` and the server functions stop the conversion with the reason
- The conversion of application statements is implemented for **Oracle** - the source which leaves the most standing after a transpilation. `WHERE ROWNUM <= n` becomes `LIMIT n` in the query shape where the two mean the same thing (the ORDER BY in the subquery, the ROWNUM around it) and is refused where they do not; `TRUNC(d, 'MM')` is written with the field PostgreSQL knows rather than with the format model of Oracle, and `ADD_MONTHS` becomes the addition of an interval. The hierarchical query, `ROWID`, a table read over a database link, `MONTHS_BETWEEN`, `SYS_CONTEXT` and the `DBMS_` packages stop the conversion with the reason, and the empty string, the arithmetic of `SYSDATE`, an identifier quoted in upper case and `TRUNC()` with one argument are converted and reported. Two defects of the view conversion came out with it: an outer join condition under an `OR` was moved into the `ON` clause anyway, and a `(+)` written inside a call was dropped without a word - both turned an outer join into an inner one, and both are reported now
- The conversion of application statements is implemented for **MySQL** and **MariaDB** - two connectors and one dialect, so the conversion stands once and is mixed into both, together with the view conversion and the function mapping each of them carried a copy of. This is the source whose dialect the parser really models, so the work is not the transpilation but the line around it: a statement which cannot be parsed used to be handed back in the dialect of the source as if it had been converted, and is an error now; `CONCAT_WS` was wrapped in a CASE which answers NULL where MySQL skips it, `DATEDIFF` became a cast PostgreSQL refuses, `DAYOFWEEK` became a function which does not exist - all of them are rewritten now, together with `TIMESTAMPDIFF`, `SUBSTRING_INDEX`, `MID`, `DAYNAME`, `WEEKDAY`, `QUARTER`, `UNIX_TIMESTAMP` and the casts to an unsigned integer. What PostgreSQL has nothing for - the address, encoding, hash, session and JSON functions - stops the conversion with the reason, and what is converted and still means something else - `REGEXP` against `~`, `'0000-00-00'`, `JSON_EXTRACT` - is reported as a warning above the statement
- The conversion of application statements is implemented for **IBM Informix**, the source with the most spellings of its own: `FIRST n` and `SKIP n` are moved to the end of the statement as `LIMIT` and `OFFSET`, the `OUTER(t)` of a FROM clause becomes a `LEFT OUTER JOIN` with the conditions which belong to it, `TODAY` and `CURRENT YEAR TO SECOND` become `CURRENT_DATE` and `CURRENT_TIMESTAMP`, `7 UNITS DAY` becomes an interval, `MATCHES` becomes `LIKE` or `SIMILAR TO` with its wildcards rewritten, `c.name[1,3]` becomes `SUBSTR`, the `DATETIME`/`INTERVAL` literals are written the way PostgreSQL writes them, `sysmaster:sysdual` is left out, and `DECODE`, `MDY`, `EXTEND` and `LAST_DAY` are written out - `DECODE` with `IS NOT DISTINCT FROM`, because a NULL matches a NULL there and `x = NULL` never does. What PostgreSQL has nothing for - `ROWID`, `DBINFO()`, `HEX()`, `MONTHS_BETWEEN()` - stops the conversion with the reason rather than being handed back unconverted
- The conversion of application statements is implemented for the three Db2 flavours - LUW, for i and for z/OS. They are three connectors and one SQL dialect, so the part which reads that dialect stands once and is shared by all three: the special registers written without parentheses, the labelled durations (`+ 12 MONTHS`), the isolation clause `WITH UR`, the optimizer hints, `SYSIBM.SYSDUMMY1` and `DAYS(a) - DAYS(b)`. Every clause which has no counterpart and is removed is reported as a warning above the statement, and a duration counted by a column is reported rather than guessed at. Two defects of the view conversion came out with it: `YEAR(x)` and its siblings were never converted for any Db2 flavour, and the function mapping of Db2 for i was a drifted third copy whose lookup never matched anything at all
- The conversion of application statements ends with a summary of its own, the way a migration does: what was read, how many statements each file held and what became of each of them, every statement which needs a person named with the place it stands in and the reason, the refusals grouped by the gate which refused them, the warnings with the count of the blocking ones, what the target test answered and which files were written
- The gates which decide whether a statement may be converted at all were sharpened by the query corpus of the test suite, which holds the statements an application of each source really sends: a statement which takes locks is refused from its text now rather than from the parsed statement - `holdlock` written where a parser expects an alias does not parse at all - reading a sequence is refused whatever the dialect calls it (`nextval('s')`, `seq.NEXTVAL`, `NEXT VALUE FOR seq`), `SELECT ... INTO TEMP` of Informix and `SELECT ... INTO :hostvar` are refused although no parser models them, the data change table reference of Db2 (`SELECT ... FROM FINAL TABLE (INSERT ...)`) is refused, and the deny list of the writing statements was extended by `REPLACE`, `LOAD`, `UNLOAD`, `IMPORT`, `COPY`, `REFRESH TABLE`, the cursor statements and the maintenance statements. A bare `VALUES` is a read and is converted now
- Added the conversion of the SELECT statements an application holds as text. `credativ-pg-migrator --config c.yaml --convert-queries` is a step over a migration which is already done - it creates nothing, moves no data and writes to neither database: it reads the files of statements you name, converts every SELECT with the same connector code which converted your views, asks the target whether the result is valid there, and writes one output file per input file with the outcome of every statement in it, plus a machine readable sidecar. Only SELECT is converted, and that is enforced four times over - by the parser of the source dialect, by the text itself, by the constructs which begin with SELECT and still write (`SELECT ... INTO`, a data-modifying CTE, `FOR UPDATE`, `nextval()`), and once more on the converted statement in the dialect of the target - with the test transaction read only and rolled back behind all four. A statement which cannot be converted is reported as such and commented out; nothing is ever handed back unchanged as if it had been converted. The bind parameters of the application come back exactly as they were, and a conversion which moved them into another order - `TOP (?)` becomes `LIMIT $1` at the other end of the statement - is reported as a blocking warning. Implemented for MS SQL Server and Sybase ASE sources

And the fixes of the 0.15 maintenance branch, carried over to the current code - written the way this code is built, and only where the same thing had not already been fixed differently in the meantime.
...
- The three settings whose values include the word `off` - `migration.validate_objects`, `query_conversion.target_test` and `query_conversion.output.sidecar` - are read as the words they were written as. YAML reads a bare `off` as `false`, so writing the documented value the way it reads best used to stop the run at the start; and in the anonymization workflow a parameter written as text is read as what it says, so `pass_original: "false"` no longer passes the original value into the call of the `anon` extension
