...
- 2026.07.30
  ...
  - Fix - SQL Anywhere Connector: Populated `get_sql_functions_mapping` with SQL Anywhere date/string/utility function and keyword conversion rules (mapping `current date`, `current time`, `current timestamp`, `current user`, `getdate()`, `today()`, `isnull()`, `stuff()`, etc. to standard PostgreSQL equivalents). Integrated `apply_sql_functions_mapping` into `convert_default_value` and `convert_view_code`, resolving DDL syntax errors (`syntax error at or near "date"` on `DEFAULT current date::DATE`).
