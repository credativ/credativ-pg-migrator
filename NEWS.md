...
- The `[ PARTITIONING ]` block of the closing summary is printed - it was absent from every summary of every run, because the cursor it is handed had been closed by the block above it and it failed silently
- The objects table's `Table Partitions` row is now `Partitioned Tables`, says what happened to each scheme, and no longer reads `0` for a run which created 58 partitions
- The pre-migration analysis now makes its checks for a source read from DDL - Db2 for z/OS and Db2 for i - instead of being skipped whole along with the measurements which really do need a live instance
- Informix no longer delimits an identifier with double quotes, which it reads as a string literal - the probe `target_partitioning` uses to size a range of partitions answered the column NAME instead of its smallest and largest value
- A `target_partitioning` entry whose bounds could not be read is now blocking rather than a warning: what it produced was a table partitioned with zero partitions under it, refusing every row, at the end of a run which printed "Migration Done"
