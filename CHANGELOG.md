...
- 2026.07.29
  ...
  - Feature - Planner / PostgreSQL Connector / Config: Added target PostgreSQL extension verification and auto-creation during pre-migration analysis (`planner.check_target_capabilities`). Supports configuring `migration.required_extensions` (e.g. `uuid-ossp`, `pgcrypto`, `postgis`) and automatically infers extension dependencies based on configured functions (such as `uuid_generate_v4()` requiring `uuid-ossp`). `check_and_create_extension` verifies extension presence via `pg_extension`, attempts `CREATE EXTENSION IF NOT EXISTS` if missing, and stops the migrator with a clear blocking report before any target objects are created if creation fails.
