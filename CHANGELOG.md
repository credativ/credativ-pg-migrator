...
- 2026.07.24
  ...
  - Fix - Planner/Default Values: Anchored the auto-injected SQL function mapping regexes in the `default_values_substitution` table (`(?i)^...$` instead of the unanchored `(?i)...`). Previously a generic function mapping such as `suser_name() -> current_user` was stored as an unanchored regex and matched via a substring search, so a composite column default like `'['+suser_name()+'@'+host_name()+']'` was collapsed to the bare replacement `current_user`, discarding the surrounding brackets, `@`, `host_name()`, and concatenation. Because these rows are flagged with `(?i)`, they also took precedence in the query `ORDER BY` and silently overrode user-defined whole-value substitutions from the configuration file. With the anchored pattern, a function mapping only replaces a default that is exactly that function; function-in-expression defaults are instead translated token-wise by each connector's `convert_default_value` / `apply_sql_functions_mapping`, and user-defined whole-value substitutions are honored. The change is shared by all connectors and is safe because every connector already performs token-level default translation. (#95)
  - Fix - PostgreSQL Connector/Defaults: Prevented niladic SQL keyword-functions from being emitted as quoted string literals in `CREATE TABLE` column defaults. For string columns (`CHAR`/`VARCHAR`/`TEXT`) the default was only left unquoted when it contained `||`, `(`, `)`, or `::`; a keyword-function without parentheses such as `current_user` (produced when Sybase `suser_name()` is mapped to `current_user`) failed all of those signals and was quoted, so PostgreSQL stored the constant text `'current_user'::character` instead of evaluating the function. The generator now also recognizes the parenthesis-less niladic keywords (`current_user`, `current_role`, `session_user`, `user`, `current_catalog`, `current_schema`, `current_timestamp`, `current_date`, `current_time`, `localtime`, `localtimestamp`) as expressions and leaves them unquoted, while genuine string literals are still quoted.
  - Fix - Planner/Column Types: Corrected the varchar/char to text length promotion so that `varchar`-family columns are governed exclusively by `varchar_to_text_length` and `char`-family columns exclusively by `char_to_text_length`. Because `CHAR` is a substring of `VARCHAR`, the previous `'CHAR' in coltype` test also matched `varchar`/`univarchar`/`nvarchar` columns, so when a `varchar` column was shorter than `varchar_to_text_length` it fell through to the `char` branch and was promoted to `TEXT` based on the (usually much smaller) `char_to_text_length`. This notably affected Sybase ASE `univarchar` columns (mapped to `VARCHAR`), which were converted to `TEXT` regardless of the configured `varchar_to_text_length`. The varchar-family type is now matched first and excluded from the char branch, and each branch is guarded by its own limit `>= 0` check so an unset limit (`-1`) never triggers a conversion. The change is shared by all connectors.
