...
class MySQLConnector(MySqlQueryConversion, MySqlPartitioning, DatabaseConnector):
    ...
    PARTITIONING_LOG_NAME = 'mysql_connector'
        ...
        try:
            ...
            if source_table_rows_limited == 0:
                ...
                if target_table_rows == 0 or data_conflict_action in ('merge_keep_target', 'merge_keep_source', 'replace'):
                    ...
                    self.config_parser.print_log_message('INFO', f"mysql_connector: migrate_table: Worker {worker_id}: Source table {source_table_name}: {source_table_rows_limited} rows / Target table {target_table_name}: {target_table_rows} rows - starting data migration.")
                    ...
                    self.config_parser.print_log_message('INFO', f"mysql_connector: migrate_table: Worker {worker_id}: Target table {target_table_name} has {target_table_rows} rows and data_conflict_action is '{data_conflict_action}'. Skipping data migration.")
    ...
    def fetch_indexes(self, settings):
        ...
        try:
            ...
            # Check if EXPRESSION column exists in INFORMATION_SCHEMA.STATISTICS (MySQL 8.0+)
            ...
            for index_name, index_info in table_indexes.items():
                ...
                if not cols_joined.strip():
                    if getattr(self, 'config_parser', None):
                        self.config_parser.print_log_message('WARNING', f"mysql_connector: fetch_indexes: Index '{index_name}' on table '{source_table_name}' has no columns or supported expressions - skipping.")
                    continue
    ...
    def get_generated_columns_count(self, table_schema: str) -> int:
        ...
        try:
            ...
            self.config_parser.print_log_message('WARNING', f"mysql_connector: get_generated_columns_count: Could not count generated columns: {e}")
            return 0
