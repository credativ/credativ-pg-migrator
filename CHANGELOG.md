...
- 2026.08.27
  ...
  - New - **Partitioning, Oracle to PostgreSQL.** The second source of `development/PARTITIONING_STRATEGY.md` §2.4, and the one which was placed there because everything a report about partitioning has to be able to say exists in one Oracle database. `ALL_PART_TABLES`, `ALL_PART_KEY_COLUMNS`, `ALL_SUBPART_KEY_COLUMNS`, `ALL_TAB_PARTITIONS` and `ALL_INDEXES` are read, and the `[ PARTITIONING ]` block of the pre-migration analysis says what each partitioned table is partitioned by before anything is created. With `migration.source_partitioning: preserve` - the default - a RANGE, LIST or HASH scheme is built on the target: Oracle's `VALUES LESS THAN (x)` and PostgreSQL's `FOR VALUES FROM (a) TO (b)` are the same scheme written twice, so a range maps exactly, `TO_DATE(' 2024-01-01 00:00:00', 'SYYYY-MM-DD HH24:MI:SS', 'NLS_CALENDAR=GREGORIAN')` becomes `'2024-01-01 00:00:00'`, and a bound this migrator cannot write as PostgreSQL - an interval, an expression - **stops the run instead of being guessed**, because a bound guessed wrong is a partition which quietly takes the rows of the one beside it. **Read against a mocked catalogue only: this has not been run against a live Oracle instance.**

  - Change - Oracle: **a sub-partitioned second level is not carried over, and the run says how many segments were left behind.** A monthly range sub-partitioned by hash into 16 is 960 relations on the target, each with its own relcache entry, statistics and indexes, and autovacuum with 960 tables to think about instead of 60 - while what the hash level buys on Oracle, I/O spread across devices and partition-wise joins on the sub-key, is not what it buys here. The first level is built, both levels are recorded in the protocol, and the closing summary says `first 1 level(s) preserved, 1 NOT carried over` rather than `scheme of the source preserved`.

  - New - Oracle: **what stops being automatic is said out loud.** An INTERVAL scheme keeps the partitions which exist - they are ordinary range partitions - and the run reports, with the interval expression and the count, that Oracle creates the next partition by itself and PostgreSQL does not, so every INSERT past the newest bound will be refused: write a `target_partitioning` entry, or hand the maintenance to `pg_partman`. An AUTOMATIC LIST gets the same finding. A HASH scheme carries over its **count** and not its placement, because Oracle hashes with its own function and PostgreSQL with its own - nothing is lost by it, the rows go in through the parent and the target routes each of them, and a reader would otherwise assume the opposite. Global indexes, the tablespaces the partitions sit in, and a bound carrying a time of day (an Oracle `DATE` holds one and the `date` it becomes does not) are each named.

  - Change - Oracle: **REFERENCE and SYSTEM partitioning stop the run** rather than arriving as something else. Neither has a partitioning key which could be written into a `PARTITION BY` clause - the first takes its partitions from the parent through a foreign key, the second from the application on every INSERT - and the message names both ways out. They are refused only where the scheme would be built: `source_partitioning: flatten` migrates the table as one ordinary table and refuses nothing.

  - Fix - **A partitioning key which does not contain the primary key is refused for a scheme carried over from the source, and not only for one somebody configured.** This is the gap a PostgreSQL source could never show: Oracle keeps a `PRIMARY KEY (ORDER_ID)` on a table partitioned by `ORDER_DATE` in a **global index**, which is legal there and ordinary there, and PostgreSQL has no global index. The table would have been created, the rows loaded, and `ADD PRIMARY KEY` refused at the very end of the run. Every primary key, unique constraint and unique index of a preserved table is checked before anything is created, and the message names the key, its columns and what to write instead.

  - Fix - **The partitioning columns of a `target_partitioning` entry are written in the names the target has.** The entry is written in the names of the source and went into the `PARTITION BY` clause unconverted, so an Oracle entry naming `ORDER_DATE` under `names_case_handling: lower` produced a clause naming a column which is not there. The same distinction now runs through the whole feature: a scheme carries the key and the bounds as the **source** wrote them, for the report and the protocol, and as the **target** takes them, for the `CREATE TABLE`.

  - Fix - The pre-migration analysis asked the source connector about **every partition of every scheme** - one catalogue query per partition, 960 of them for a composite Oracle table - although the plan needs an answer only for a partition which is itself partitioned. It asks about those.

  - Fix - The type check of a `date_range` reads past a precision, so an Oracle `TIMESTAMP(6) WITH TIME ZONE` is recognised as a column which carries a calendar instead of being refused for the way it is spelled; and a column is found in the facts of a table however the entry spells its name. A lookup which is not case-insensitive answered nothing for a column which is there, and a check skipped in silence reads exactly like one which passed.

  - Fix - **The `[ PARTITIONING ]` block of the closing summary is printed.** It was absent from every summary of every run. The block is handed the cursor its caller holds, and by the time it gets there the caller has rebound and CLOSED that name several times over - the remote-object substitutions are the last to do it - so every query it made failed with *"cursor already closed"*. It failed silently, returning an empty list, which is indistinguishable from a migration in which nothing was partitioned; the block simply was not there. It gets a cursor of its own now, it says so when it cannot be built, and a migration with nothing partitioned on either side gets one line saying that rather than nothing at all.

  - Fix - **The partitioning of a run is recorded in the protocol tables even when the plan is not built by the analysis.** `record_partitioning()` was called from the analysis, and a source read from DDL has its extracts parsed afterwards - so for Db2 for z/OS and Db2 for i the two protocol tables stayed empty, and the summary reported a z/OS `ORDERS` really partitioned by range over `ORDER_DATE` as *"partitioned by target_partitioning"* with no source scheme at all. It is recorded where the plan is BUILT now, which is the plan the run actually uses.

  - Fix - **A table partitioned by `target_partitioning` is recorded as the target's scheme.** `record_partitioning()` skipped every decision which was not `PRESERVE`, so `<protocol>_target_table_partitioning` held nothing for exactly the tables the configuration asked to partition - the summary counted their partitions out of the statements instead of reading them, and the generated partition names were nowhere. They are recorded where the bounds are worked out, which is the only place they exist.

  - Change - **`Table Partitions` in the objects table is called `Partitioned Tables`.** The row counts rows of `<protocol>_source_table_partitioning`, which is one per LEVEL of one partitioned table - so it never counted partitions, and a run which created 58 of them from `target_partitioning` showed `Table Partitions 0 | 0 | 0`. Its Details column now says how many schemes were carried over, how many were flattened and how many the configuration added, and points at the `[ PARTITIONING ]` block - wherever that block went, which is not "below" when `summary.report_filename` is set. Its Success and Failed columns read `-` rather than `0`: nothing sets a success flag on what was read from the source.

  - Fix - Db2 for z/OS and Db2 for i: **the checks of the pre-migration analysis are made.** The whole analysis was skipped for a source read from DDL. The MEASUREMENTS in it need an instance to ask - the version, the size, the top tables by rows - and a DDL migration has none; the CHECKS do not, and they were skipped along with them. So the one `target_partitioning` entry these two sources cannot carry out, the one which generates its partitions from a `date_range`, was never looked at: the refusal was written, documented in the example configuration, covered by tests, and unreachable. It is reached now, and it says what it always meant to say - *"this source has no database to ask: its structure comes from DDL files and its data from CSV files"* - rather than the run creating a partitioned table with nothing under it.

  - Fix - Db2 for z/OS and Db2 for i: **"not parsed yet" is not "there is none".** The DDL of these two sources is parsed into the migration AFTER the analysis runs, so at the point the analysis asks, `ddl_tables` is empty. Left alone it reported *"no table of the source schema is partitioned"* about a schema whose `ORDERS` is partitioned by range over `ORDER_DATE` in its own `CREATE TABLE`, and it reported it before the clause had been looked at. It says the DDL has not been parsed at this point now. For the same reason the plan built during the analysis is thrown away rather than kept: the plan is normally read once so the report and the run cannot disagree, and this one would have made the run agree with a report which established nothing - `ORDERS` arriving as one ordinary table. An entry naming a table is no longer refused as *"the source schema does not hold"* it either, when the table list is the thing that was not read.

  - Fix - Informix: **an identifier is not delimited with double quotes.** `DELIMIDENT` is off by default and Informix reads `"rate_date"` as a STRING LITERAL, so the query `target_partitioning` builds to find the smallest and the largest value of a column - inherited from the base connector, in the quoting of PostgreSQL - was not merely refused. `select min("rate_date") from informix.currency_rates` answers `'rate_date'`: the column NAME, which is a perfectly good answer to a question nobody asked, and a range of partitions generated from it would have been built out of it. What saved this run is that the table half of the same query, `"informix"."currency_rates"`, is a syntax error - so the bounds were reported as unreadable rather than read wrong. The connector delimits nothing now, which is what the rest of it has always done.

  - Fix - **A `target_partitioning` entry whose bounds could not be read is BLOCKING.** It was a warning, and the outcome it produces is the one three neighbouring branches already call blocking: a table created `PARTITION BY RANGE` with **zero** partitions under it, which refuses every row. An Informix run took this path for real - it created `currency_rates` partitioned and empty, refused all 442 rows with *"no partition of relation currency_rates found for row"*, and finished by printing **Migration Done**. Only the reason differs between "there is no instance to ask" and "the instance refused the question", and the reason is not what the migration pays.

  - Fix - Informix: **the report no longer says a table is flattened and preserved in the same breath.** The note *"the fragments of X really are a RANGE over Y, so the target is given the same scheme"* is written by the connector, which reads the source and does not decide what happens to it - `source_partitioning` decides that, per table - so it contradicted the planner's own line for every table set to `flatten`. It says what was read now, and the planner says what happens to it.

  - Fix - Db2 for LUW: **an ordinary table is no longer read as a partitioned one.** `SYSCAT.DATAPARTITIONS` holds a row for **every** table there is - a table which was never partitioned gets a single data partition named `PART0`, with `LOWVALUE` and `HIGHVALUE` both NULL - so "it has a row in DATAPARTITIONS" says nothing at all. Reading it as the test reported all 25 ordinary tables of a schema as `RANGE` with one partition and no key columns, and then refused the whole migration with 24 blocking issues saying *"a RANGE scheme whose key columns the source does not hold"* about tables which are not partitioned in any sense. The test is `SYSCAT.DATAPARTITIONEXPRESSION`, which holds the partitioning key and holds rows only for a table which really has one; the partition count is the tie-breaker for the case where the expression could not be read, which stays a blocker rather than becoming a quiet "not partitioned". Found by running the migtest example of `credativ-pg-migrator-tests` against a live Db2 through `system_catalog: SYSCAT` - the shipped example reads `SYSIBM`, under which none of this code runs.

  - Fix - **`target_partitioning` works on a source which has no partitioning of its own.** The two halves of this feature are independent - reading how the source partitions is per connector, and `target_partitioning` reads a configuration and writes PostgreSQL - and the planner did not treat them that way: a source which **declares it has no partitioning** (SQLite, SQL Anywhere) or whose connector does not read it returned no partitioning plan at all, so every `target_partitioning` entry against such a source was refused by the pre-migration analysis with *"names the table X, which the source schema does not hold"* - and would have been silently never applied had it passed. The plan is built over empty schemes now. Those two sources are exactly the ones most likely to be GIVEN a scheme, which is what made this worth finding.

  - Fix - **`target_partitioning` names its table and its columns however the source spells them.** `table_settings` has always matched a table name case-insensitively; `target_partitioning` matched it exactly, so an entry naming `currency_rates` against Oracle or Db2 - whose catalogues answer `CURRENCY_RATES` - was refused as a table the schema does not hold. The columns had the same problem one step later and worse: they are used to ask the SOURCE for the smallest and largest value of the partitioning column, so `min("rate_date")` against Oracle is ORA-00904, in the middle of preparing the table rather than in the analysis. Both are resolved against the real names of the source now, and the clause of the target is written from that spelling through `names_case_handling` - three spellings of one name, each used where it belongs.

  - Fix - Informix: **the fragment expressions are read at all.** `sysfragments.exprtext` comes back **NUL-terminated** - the remainder fragment as `'remainder\x00'` and an expression as `"order_date < MDY(1,1,2023)\x00"` - and `str.strip()` does not remove a NUL, so every fragment expression of every Informix table was refused as *"not a range or a list over one column"* and every fragmented table stopped the run. Found by running the migtest example of `credativ-pg-migrator-tests` against a live Informix.

  - Fix - **A partition which is not a table is no longer reported as one the filters left out.** The warning *"the partition X is not selected by include_tables / exclude_tables, and it is created anyway"* is about a table the filters could have selected and did not, and that only exists where a partition is a relation of its own - which is PostgreSQL and nothing else. On every other source it was one line of noise per partition: an Oracle INTERVAL table of 55 months produced 55 of them. The scheme says which kind of source it came from now (`partitions_are_tables`).

  - Fix - Sybase ASE: **the partitioning is read from `sp_helpartition` and not from `syspartitions`.** Run against a live ASE 16.0 SP02 for the first time, and `syspartitions` turned out to have **no partition condition column at all** - its columns are name, indid, id, partitionid, segment, status, datoampage, indoampage, firstpage, rootpage, data_partitionid, crdate, cdataptnname, lobcomp_lvl and ptndcompver - while `syspartitionkeys` has only (indid, id, colid, position). The bounds of a range or a list scheme are in neither. `sp_helpartition` holds all of it and more: four result sets carrying the method ('range' / 'hash' / 'list' / 'roundrobin'), the key columns, one row per partition with its segment and row count, and the conditions - which have no key of their own and are matched to the partitions by POSITION, and only where the two are the same length, because a hash scheme answers a single NULL row there whatever its partition count. `syspartitionkeys` is also ordered by `position` now and not by `colid`: the second is right by accident for a key of one column and wrong for every other. What the design had built for still happens where `sp_helpartition` cannot be run - the scheme is reported in full with its method stated as not known, and nothing is built out of a read which did not happen.

  - Fix - MS SQL Server: **a partition boundary is converted by the server and not by the driver.** `sys.partition_range_values.value` is a `sql_variant`, and this connector registers an ODBC output converter for that type so that a `sql_variant` COLUMN of a migrated table arrives as text - which fired on the boundary value too and turned the `smallint` boundary 2023 into the single character U+07E7, its own bytes read as a string. Every RANGE scheme of every SQL Server source was therefore given nonsense bounds, or refused for holding them. The boundaries are read with `CONVERT(nvarchar(4000), prv.value, 126)` now, so the driver never sees a `sql_variant` there; whether a bound is then written bare or quoted is decided by the **declared type** of the partitioning column rather than by what the text looks like, because a `varchar` function whose boundaries happen to be '100' and '200' is a range over two strings. Found by running the connector against SQL Server 2022.

  - Change - MariaDB: a table partitioned `BY SYSTEM_TIME` is refused with the reason it really has - PostgreSQL partitions by RANGE, LIST and HASH and by nothing else, and SYSTEM_TIME rotates the history rows of a system-versioned table out of the current partition, which is a feature of the versioning rather than a way of dividing rows by their values. It used to be refused for having an expression key, which is true of it and is not why it cannot be built.

  - New - **`docs/partitioning.md`**, a reference page for the whole feature with **one section per source database**: what each engine has, which catalogue it is read from, what `source_partitioning: preserve` carries over, what is reported rather than reproduced, and what stops the run. It also states plainly what the feature matrix can only mark with a letter - that only the PostgreSQL source has been run against a live server of its own engine, and that Sybase ASE's catalogue names were never verified at all. The two long footnotes of `FEATURE_MATRIX.md` point at it instead of repeating it, and the sample outputs on the page are generated from the code rather than written by hand.

  - New - **Partitioning, MS SQL Server to PostgreSQL - and with it, all twelve sources.** A SQL Server partitioning is not written on the table: it is a partition **function**, which says where the boundaries are, and a partition **scheme**, which maps the partitions onto filegroups. The ranges are reproducible and the filegroup placement is not - it is named per table and not carried over, along with the per-partition compression, which PostgreSQL has no counterpart for. `sys.partitions` keeps an exact row count per partition, so the skew of a scheme is reported before anybody reproduces it, and a **non-aligned** index is named for what it is: SQL Server's answer to the question Oracle answers with a global index, which PostgreSQL has neither of. **Read against a mocked catalogue; not run against a live SQL Server.**

  - New - MS SQL Server: **`RANGE LEFT` is converted rather than copied, which is the whole difficulty of this source.** One bit of one catalogue row decides which side of a boundary the boundary value falls on. `RANGE RIGHT` puts it in the partition above - which is exactly what PostgreSQL's `FROM (a) TO (b)` means, inclusive below and exclusive above - and maps with nothing done to it. `RANGE LEFT` puts it in the partition **below**, which is the opposite at both ends, so every bound moves to the next value of the type. A scheme copied bound for bound out of a RANGE LEFT function is short by exactly the boundary value at every boundary - the rows do not fail to load, they load into the partition next door, and no error message would ever mention it. A `RANGE LEFT` function over a `datetime` is refused rather than shifted by a guess: SQL Server counts a datetime in units of 1/300 of a second, which is why such a boundary is written `'…23:59:59.997'` in the first place, and the value after that is not something the bound says.

  - New - MS SQL Server: **a nullable partitioning column is named for what it costs.** SQL Server puts a row whose key is NULL in the lowest partition; PostgreSQL puts it in none at all - a RANGE partition takes no NULL and only a DEFAULT partition does, which a scheme carried over from a source has not got. Whether the column really holds one is not something the catalogue answers, so it is said rather than refused. A catalogue whose partition count and boundary count do not add up - a function with n boundaries makes n + 1 partitions - builds nothing, because a scheme read wrongly is a different scheme wearing the right number of partitions.

  - New - **Partitioning, Sybase ASE to PostgreSQL.** The sixth source of `development/PARTITIONING_STRATEGY.md` §2.4, and the one that document wants most and knows least about: the engagement behind this repository's query conversion work is a Sybase ASE one, and the same row of §2.4 admits that its catalogue names were **never verified against a live server**. That admission is the design here rather than a footnote. `syspartitions` and `syspartitionkeys` are read for the partition names, the segments they sit in, their row counts and the key columns; ASE's semantic partitioning spreads a table over segments for I/O across devices and for parallel scans, so where a partition sits is half the reason it exists and none of the placement is carried over. `VALUES <= (100)` is an inclusive upper bound and becomes `TO (101)`, the same conversion Db2's `ENDING AT … INCLUSIVE` gets - the arithmetic now stands once and both connectors share it - and where the column type has no next value the scheme is refused rather than moved by a guess. **Read against a mocked catalogue; not run against a live Sybase ASE, and the feature matrix marks it `?` rather than `yes` for that reason.**

  - New - Sybase ASE: **a partition condition which could not be read is not one which is empty, and the two are never confused.** The condition is what a RANGE or a LIST scheme would be built from and it has no place this migrator can point at with confidence, so it is tried - the column `syspartitions` is documented to carry it, then `sp_helpartition`, ASE's own report, whose result set is read by the *names* of its columns because their order differs between releases. When neither answers, the scheme is reported in full - how many partitions, what they are called, which segments, how many rows - with its method stated as **not known**, and the run stops. RANGE, LIST and HASH are built from three different things, and a HASH built out of a RANGE nobody could read is the one wrong answer with no symptom at all: every row would be loaded into the wrong partition and not a single step of the run would fail.

  - New - Sybase ASE: **ROUND ROBIN has no key of any kind and stops the run** - ASE puts each row in the next partition in turn to spread the writes over segments, and PostgreSQL routes a row by its value. A HASH scheme carries over its count and not the placement of a row, because ASE hashes with its own function. A LIST key over more than one column is refused, because PostgreSQL's takes exactly one. The method itself is worked out from two things which are not in doubt - whether the table has partitioning key columns at all, and the shape of the conditions - rather than from a catalogue column this migrator would have to guess at.

  - New - **Partitioning, MySQL and MariaDB to PostgreSQL.** The fifth source of `development/PARTITIONING_STRATEGY.md` §2.4, and one implementation for the pair - the two are one dialect, as their query conversion already is, so the reading is one mixin and every test of it runs against both connectors. `information_schema.PARTITIONS` holds the whole scheme in one query: the method, the expression, the bound of every partition and the rows in it. `RANGE` and `RANGE COLUMNS` map exactly - `VALUES LESS THAN (x)` is an exclusive upper bound over contiguous ordered partitions, which is what PostgreSQL says with both of its ends - and so do `LIST` and `LIST COLUMNS`, except over more than one column, which PostgreSQL's LIST key does not take. Sub-partitions are not carried over and the segments left behind are counted, with the rows of each segment added back into the partition it belongs to. **Read against a mocked `information_schema`; not run against a live MySQL or MariaDB.**

  - New - MySQL/MariaDB: **a hash scheme carries over its count and not the placement of a row.** MySQL hashes an integer expression with its own function, `LINEAR HASH` uses a power-of-two variant of it, and `KEY` uses the internal hashing of the storage engine; PostgreSQL hashes the column value with its own. The same column and the same partition count therefore do not put the same rows in the same partition. Nothing is lost by it - the rows go in through the parent and the target routes each of them - and it is said per table, because a reader would assume the opposite. `PARTITION BY KEY()` with **no** column list means the primary key and `information_schema` writes no expression for it; that one is refused rather than guessed at.

  - New - MySQL/MariaDB: **a partitioning expression which is not a bare column stops the run**, and the reason is not that PostgreSQL cannot express it. It can - and a table which is partitioned by an expression can then have no primary key and no unique constraint **of any kind**, because every unique constraint of a partitioned table has to contain all of its partitioning columns and no constraint can contain an expression. MySQL's rule runs the other way and requires every unique key to contain the columns *of* the expression, which is why `PARTITION BY RANGE (YEAR(hired))` sits happily beside `PRIMARY KEY (id, hired)` there - so the tables which use the commonest MySQL idiom of all are precisely the tables with a key to lose, and building it would give a target which loads its rows and then refuses its primary key at the very end of the run. The message names both ways out, and one of them is short: `target_partitioning` with `partitioning_columns: hired` and `date_range: year` builds that exact scheme from the data.

  - New - **Partitioning, Informix to PostgreSQL.** The fourth source of `development/PARTITIONING_STRATEGY.md` §2.4, and the one whose verdict there was written about the report rather than about the reading: *"the case where the honest report is none of this should be reproduced, which is worth more than a translation."* A table is **fragmented** across dbspaces to spread its I/O over devices, so a fragment is a place and not a class of row - and PostgreSQL does not need a partition to put a table in a tablespace. `sysfragments` is read in full: the strategy, the expression of every fragment, the dbspaces they sit in, and the rows per fragment, so that a scheme where 95% of the rows sit in one fragment is seen before anybody reproduces it. Empty fragments are counted, and a table nobody has run `UPDATE STATISTICS` over says its spread is **not known** rather than reporting zeros. **Read against a mocked `sysfragments`; not run against a live Informix.**

  - New - Informix: **a fragmentation which really is a partitioning is carried over.** `FRAGMENT BY EXPRESSION order_date < DATE('2023-01-01') IN dbs1, order_date < DATE('2024-01-01') IN dbs2, REMAINDER IN dbs3` is a range scheme spelled the way Informix spells one, and it becomes one. What makes it readable is first-match: Informix evaluates the fragments in order and stops at the first true one, so the second fragment really holds 2023 - the first already took everything below it. Read literally the second expression covers the first, and copying the pair across would be refused by the target as overlapping partitions. `col IN (…)` and `col = v` become a LIST scheme the same way, and `REMAINDER` becomes the DEFAULT partition.

  - New - Informix: **what has no counterpart stops the run rather than arriving as something else.** **ROUND ROBIN** puts each new row in the next dbspace in turn - there is no key, no expression and nothing about a row which decides where it goes, and PostgreSQL routes a row by its value. **HYBRID** is two strategies at once. An arbitrary boolean expression is a rewrite and not a translation and is refused **with its own text quoted**, so that whoever reads the message can see what was refused rather than being told that something was; so are fragments over different columns, fragments which overlap, two bounds this migrator cannot order, and a bound which is not a literal - `TODAY` in a bound is a boundary which moves, and a partition whose boundary moves is not the partition the source had.

  - New - **Partitioning, IBM Db2 to PostgreSQL - all three flavours.** The third source of `development/PARTITIONING_STRATEGY.md` §2.4, built as one family: Db2 for LUW out of a live `SYSCAT`, Db2 for z/OS and Db2 for i out of the `PARTITION BY` clause of their DDL extracts, which for those two connectors is the catalogue. A range scheme is carried over, and the trap of the whole family is the bound itself: **Db2 writes `ENDING AT (x) INCLUSIVE` and means x is IN the partition, PostgreSQL's `TO (b)` means b is not, and INCLUSIVE is Db2's default.** A scheme written `STARTING '2024-01-01' ENDING '2024-12-31'` copied bound for bound gives a target which refuses every row of 31 December - one row at a time, at the end of a migration which has moved the rest. Each inclusive end is carried over as the exclusive bound holding the same rows, and where the column type has no next value - a `DECIMAL` with a scale, a `VARCHAR`, a `TIMESTAMP` - the scheme **stops the run** rather than being moved by a guess. **Read against mocked catalogues and mocked extracts; not run against a live Db2 of any flavour.**

  - New - Db2: **the three things Db2 calls partitioning each get their own sentence.** Only one of them is table partitioning. **DPF** spreads the rows of a table over the physical nodes of the instance by a hash of the distribution key - the key is read out of `SYSCAT.COLUMNS.PARTKEYSEQ` and named, PostgreSQL has no counterpart of any kind, and nothing about the table needs changing for it. **MDC** is a storage layout deciding which rows share a block; its dimensions are named, and the nearest PostgreSQL things are `CLUSTER` and a BRIN index, neither of which is this. A table which has one of those and no data partition is **not** reported as partitioned - it is not a partitioned table in the sense PostgreSQL means, and answering that it is would build a partitioned target for a scheme which is not one. z/OS **partition-by-growth** (`PARTITION BY SIZE EVERY 4G`) has no partitioning key at all - a partition exists because the one before it filled up - and stops the run instead of arriving as something else.

  - Fix - Db2 for z/OS and Db2 for i: **the DDL parser reads the whole `PARTITION BY` clause.** Both connectors had matched it out of the `CREATE TABLE` text for as long as they have existed, and what they matched was less than it looked: the expression did not know the word `RANGE`, so `PARTITION BY RANGE (ACCT_NUM) (…)` - the spelling of every release since Db2 9 - was **never matched at all**; and its bracket for the partition list stopped at the first closing one, so `PARTITION 1 ENDING AT (199), PARTITION 2 ENDING AT (299)` came back as one partition with the rest of the list inside it. `HASH` and `SIZE` were not recognised as anything. All of it is one shared parser now, with the brackets balanced. `PARTITION n ENDING AT (x)`, which says nothing about where the partition begins, is read with Db2's own rule that the ranges of a partitioned table space are contiguous - it starts where the one above it stopped.

  - Fix - Db2 for z/OS and Db2 for i: **the comment of a table is the table's comment again.** The partitioning of the source was written into it as free text, so a partitioned table reached the target with `COMMENT ON TABLE … 'Partition: X, Ranges: Y'`, an unpartitioned one got no comment at all, and the comment the DDL really carried - which the same parser has always stored - was read by nothing. The scheme is read as a scheme now.

  - Fix - Db2 for z/OS and Db2 for i: **a `target_partitioning` entry with a `date_range` is refused instead of ending the run in a traceback.** Both connectors are offline - the structure comes from `.sql` extracts and the rows from CSV files - so there is no instance to ask for the smallest and the largest value the partitioning column holds, and the generator would have sent a SELECT down a connection which does not exist, halfway through preparing a table. Such a source declares that it cannot be asked, and the pre-migration analysis refuses the entry before anything is created. A probe which was attempted and failed stays what it was: a check which was not made, reported as such.

  - Change - Db2 for LUW: a migration configured with `system_catalog: SYSIBM` reports the partitioning of its source as **not read** rather than answering that nothing is partitioned. The standard views describe none of the three mechanisms. It is the first place where what a connector cannot read depends on how the migration was pointed at its source rather than on the connector.

  - New - The `[ PARTITIONING ]` headline of the pre-migration analysis counts the tables partitioned on more than one level on their own - `12 of 340 table(s) are partitioned on the source, holding 640 partition(s); 4 of them are partitioned on more than one level` - and the source partitioning protocol table has a `source_partitioning_engine_specific` column holding, as JSON, what only that engine has: the INTERVAL expression, the sub-partitioning which is not carried over, the tablespaces. It is written so that it can be read; nothing in the migration reads it back.

- 2026.08.26

  - Fix - **The validator of the mapping workflow can open the connection it compares against.** `validation.target_copy` is the third connection of a run - the untouched copy of the target from before the migration, which is the only way to tell a row the migration wrote from a row which was already there. `get_db_config()` answered for it and `get_db_session_settings()` next to it refused it with `Invalid source_or_target: target_copy`; every connector asks for its session settings while it is being built, so the connection could never be opened and **every table of every mapping validation was reported as failed** - with the reason, which the validator itself says is a failure of the validation and not a measurement of the table. What the connections of a run are is written down once now, so the two cannot drift apart again, and the refusal of a name which is not one of them says which names there are.

  - Fix - **`validation.target_copy` accepts the keys the code reads from it.** The block forbade `owner` - which the validator asks for right after `schema`, exactly as it does for the target - and had nowhere to write `settings`, although it is a connection to a PostgreSQL database like any other and may need a role or a `search_path` of its own. It takes the same keys as `target` now, and a test keeps the two lists together.

  - Change - **`remote_objects_substitution` is deprecated**, and a configuration which uses it is told so at startup with what it really does: a plain search and replace over the whole statement, which rewrites a name inside a string literal or a comment as readily as one in the SQL, matches a substring rather than a name (a rule for `arch` fires inside `archive_2024`), and gives a different result depending on the order the entries are written in. It still works, and a reference to another database is still what it is for - a reference to the database being MIGRATED no longer needs it at all. `development/REMOTE_OBJECTS_SUBSTITUTION.md` records what it would take to replace it.

  - Fix - **`remote_objects_substitution` has one contract instead of two.** The accessor answered whatever the YAML happened to hold - the default was a *dict* while the schema documents a list of pairs - so a configuration written as a YAML mapping worked in the query conversion and stopped the planner on every run with `too many values to unpack`. It answers a list of pairs now, always, and a shape it cannot read is refused at startup with a message showing how to write it: a mapping, a value which is not a list, an entry which is not exactly two elements, an empty source (it would match every position of every statement) or an empty target. Two entries with the same source, two whose replacements chain into each other, and a source contained in another source are reported as what they are - the three shapes whose outcome depends on the order the list is written in.

  - Fix - **What the substitution replaced is recorded.** It fired silently in four of the five places it is applied, so an object was created reading something other than what its source text names and nothing in the run said so - not the log, not the protocol, not the summary. The five hand-written loops are one call now, which records every replacement with the object it was made in, writes it to the new protocol table `remote_objects_applied`, warns once per object, and counts it in the summary under `[ REMOTE OBJECT SUBSTITUTIONS ]`.

  - Fix - Sybase ASE and MS SQL Server: **every reference with a database in front of it is resolved, not only the first one.** The first repair walked the TABLES of the parsed statement, so a view came back half converted - the table of the `FROM` clause resolved and a function next to it still written `ccd..fn_x(a)`. Two causes, both repaired: a qualified call is a `Dot` chain ending in the call and not a table, so `ccd.dbo.fn_calc(a)`, `ccd..fn_t(x)` and the same inside a `CROSS APPLY` were never looked at; and `db..object` in an EXPRESSION position is not readable by the parser at all, so a view carrying one was never parsed and reached the target with every qualifier of every reference untouched. The omitted owner is written back before the statement is parsed - only for the database being migrated, and only where the text is SQL, so a `..` inside a string literal or a comment stays as it was written - which makes the whole family the ordinary three part name. A call is pointed at the schema of the target directly, because nothing downstream maps the qualifier of a function the way it maps the schema of a table. The walk also collects its nodes before it rewrites any of them, which is the other way a pass stops after its first hit.

  - Fix - Sybase ASE and MS SQL Server: **a table named with its database in front of it reaches the target readable.** Transact-SQL writes up to four parts - `server.database.owner.table` - and lets the middle ones be left out, which is where `ccd..table` comes from; PostgreSQL has two. The transform of both connectors read only the owner part, and for `ccd..t` sqlglot puts the database into `catalog` and leaves the owner an empty string, so nothing matched and the qualifier travelled through: a view reached the target as `FROM ccd.."batch_task"`, which is a syntax error, or as `FROM ccd."ccd"."t"` - the name of the database written in front of the schema which had just replaced it. Most such qualifiers are not remote at all: they name the database being migrated, because old Transact-SQL qualifies out of habit. The migrator knows that name from `source.database`, so the reference is recognised as one of its own tables and the qualifier is dropped - **no configuration needed**. A reference to another database or another linked server is left exactly as the source wrote it, and the object is reported by name with what it reads: a name the target refuses is better than one which silently reads a different table, and `remote_objects_substitution` is still the way to rewrite it. The whole decision is made on the parsed statement, so a database name inside a string literal or a comment is not touched. The reasoning, and what a general mechanism for the other sources would look like, is in `development/CROSS_DATABASE_REFERENCES.md`.

  - Fix - SQL Anywhere: **a chunked read is paged out of an ordered result.** `SELECT TOP m START AT k` takes a window out of the result, and which rows are in that window is only defined when the result is ordered - the connector built the `ORDER BY` and never added it to the statement, so every chunk paged an **unordered** result and rows could be read twice or missed. It was not visible in the log either, because the statement was written there before the clause was built. The clause is added where the statement really pages - a chunked read, and a resumed one, which starts past the first row even without chunking - and not where a table is read in one pass, which needs no order and would otherwise have paid for a sort of the whole table. A column SQL Anywhere cannot sort on is left out of the order and of nothing else; a table which has neither a primary key nor a sortable column is reported rather than silently paged. **A SQL Anywhere migration which ran with `chunk_size` set is worth re-checking against its row counts.**

  - Change - **`chunk_size` is documented for what it is, and no example configuration sets it any more.** The default was already `-1` (no chunking); eight of the examples recommended a value anyway, and the reference called 10x-20x `batch_size` "good values". Chunking is not a performance setting: it replaces the one SELECT which reads a table with several paged ones, every chunk repeats the sort and re-reads the rows it skips, so a chunked table is normally slower and the more chunks the worse. It is for one case - a source whose driver or server builds the whole result set before the first row arrives, where a single SELECT over a huge table exhausts client memory or the temporary space of the server. Section 8.2 of the user guide now says that, what the paging costs, that the order it pages by is the primary key or else every non-LOB column - and that where neither is unique rows can be read twice or missed - and what each of the twelve sources really does: **Sybase ASE and the two file-based DB2 connectors ignore the setting entirely**, and **SQL Anywhere paged an unordered result** until the repair above.
...
- 2026.08.25
  ...
  - Fix - **`COMMENT ON TRIGGER` was written with a schema qualified trigger name**, which is a syntax error - a trigger belongs to its table - so no trigger comment had ever been migrated. The whole phase stood in one `try/except`, so that one statement ended it and the comments of the views and of the user defined types were never attempted. Every comment is its own statement now, one which is refused is a line in the log, a comment whose object was never created is skipped rather than blamed, and the summary reports what the phase really did instead of reporting every comment as migrated.

- 2026.08.24

  - New - Query conversion: **a converted statement names the objects the target really has.** The map is read from the protocol tables of the finished migration - the schema, the table names `names_case_handling` produced, the aliases and the columns - and applied through the parsed statement, so a column named like a common word is not rewritten inside a string literal. Every table and column the statement names which the migration does not know is listed in the block and in the protocol: this is the query which reads a table `exclude_tables` left behind, said before the target answers with a bare `relation does not exist`.

  - New - Query conversion: the first warning of the catalogue - **a column whose type class changed**. `NUMBER(1,0)` becomes `SMALLINT` unless the configuration asks for `BOOLEAN`, and where it was asked for, `WHERE FLAG = 1` stops working and no test level anywhere can see it.

  - New - The routine bodies of the nine non-Transact-SQL sources were measured, and **every source declares what its bodies keep of their own names**, with a warning per routine where `names_case_handling` is not `lower`. Informix now names the tables and views of a body as the target has them, and the routine converter of Db2 for i receives the routine at all - it read the wrong key and answered the empty string for every routine of every Db2 for i migration.

  - Fix - **"Not implemented" is no longer presented as "the source has none".** A fetch which answers nothing declares which of the two it is, per connector and per kind, with what the source really has and which catalogue keeps it; the planner and the summary then say `?` and the reason rather than `0`. A stub which declares neither fails the test suite.

  - Fix - **A phase of a migration says what it really did.** Nineteen places wrote `finished OK` on the way out of a phase whatever had happened inside it, so the index migration reported success over a run in which indexes had failed. A phase which reports success is now asked whether the objects it was there for arrived, and the summary has a `Result` column. The planner also closed a row it had never opened, so the phase row of its workflow was never closed at all.

  - Fix - The closing pass over the objects no longer calls a name lookup "valid". It says what it really checked - the object is in the catalogue of the target - and the summary reads `In target: X, Missing: Y`.

  - Fix - **The validator can say "no".** Three outcomes instead of a boolean which started at `True` (`PASSED`, `FAILED`, `NOT VALIDATED`), every table in scope is asked about rather than only the ones with recorded rows, a table whose measurement crashed stays in the report with the reason, and the index and constraint counts are checks which can fail a table instead of numbers nobody read.

  - Fix - **Column defaults are no longer dropped without a trace.** MS SQL Server: the style argument of `CONVERT` decides what the value looks like and is carried over. SQL Anywhere: a constant default was thrown away as a "column reference", and the special values of its `DEFAULT` grammar - `CURRENT UTC TIMESTAMP`, `TIMESTAMP`, `LAST USER` - were not converted at all. Oracle: `SYS_GUID()` on a BYTEA column is carried over instead of dropped.

  - Fix - **A functional index no longer loses its collation.** What a collation of another engine means is decided in one place, the raw MySQL expression of an index is never handed to PostgreSQL again, and the name of a collation is not folded together with the identifiers.

  - Fix - **A byte the assumed encoding cannot read is no longer deleted from the value.** `migration.on_undecodable_bytes` decides what happens instead - `substitute` (the default, which loses no byte), `fail` or `remove` - and it is counted and reported per value. U+FFFD is never written into the target, whatever the setting: it cannot be told apart from a replacement character which was in the data.

- 2026.08.21

  - Fix - **`names_case_handling` is applied to every name a migration creates.** The routines and the triggers, the function a trigger calls, the names inside a converted view and inside a converted application statement, and the trigger function built out of the trigger's own name. An undelimited name is folded the way the *source* folds it before the setting is applied. `NEW`, `OLD`, `TG_OP` and their fields are never renamed, and the target schema is used exactly as the configuration writes it.

  - New - The protocol tables record **the name the target object really has**, next to the name of the source, for indexes, constraints, triggers and default values.

  - New - **A run stops when `names_case_handling` would make one target object out of two of the source**, naming every clash rather than only the first.

  - Fix - IBM Db2 LUW no longer converts the case of the names it reads from the source - it was the one connector which did, so the record of what was read did not say what was there.

  - New - **The `*=` and `=*` outer joins of the old Transact-SQL are converted**, for MS SQL Server, Sybase ASE and SQL Anywhere alike, in views and in application statements. A `WHERE` condition which restricts the inner table belongs to the join in these dialects and is moved into the `ON` clause; one which cannot be attributed is reported rather than left as the inner join it would otherwise be. Oracle's `(+)` written on a condition which is not the join itself is converted as well.

  - Fix - Query conversion: the JSON sidecar could not be written and stopped the run after the output file had been produced; `on_error: stop` threw away the file it had just made; every output path is decided before the first file is read; the connections of the workers are closed by the run; and the protocol table is no longer dropped at the start of every run.

  - Fix - Query conversion: four of the five textual gates read string literals and comments as if they were SQL, so a statement mentioning `INSERT` in a comment was refused and one hiding a keyword in a literal was not.

  - New - Query conversion: the migration summary has a `[ QUERY CONVERSION ]` section, `remote_objects_substitution` is applied to every source rather than to three of them, a statement is told when its `OK` rests on the `search_path` the test set, and the step compares the protocol tables with the configuration it was given - reading the map of one migration while testing against the schema of another produces output which looks right and is not.

  - Fix - Anonymization: a method parameter written as text is read as what it says. Everything out of YAML is a string, so `length: "10"` reached a method expecting a number.

- 2026.08.20

  - New - **Query conversion implemented for SAP SQL Anywhere, Oracle, MySQL, MariaDB, IBM Informix and the three Db2 flavours.** Each source converts an application statement with the same code which converts its views, so the two cannot answer differently; what a dialect writes and PostgreSQL has nothing for stops the conversion with the reason instead of being handed back unconverted.

  - New - The configuration block `query_conversion`, and a summary of the run in the shape a migration's summary has.

  - Fix - The gates which decide whether a statement may be converted were sharpened by the query corpus of the test suite: a statement which takes locks is refused from its text, reading a sequence is refused whatever the dialect calls it, the `SELECT ... INTO` forms and the data change table reference of Db2 are refused, and a bare `VALUES` is a read and is converted.

  - Fix - Every connection to the target ran without the session settings of the configuration, so objects were created under the login role instead of the configured `role` and the migrated schema ended up with mixed ownership. The settings are applied inside `connect()` now, and read from the section of the configuration the connection belongs to.

  - Fix - A restriction from `data_migration_limitation` meant different things to the planner, the orchestrator and the validator. All three ask one resolver now, and an entry may carry a row limit as an optional fourth element.

  - Fix - `varchar_to_text_length` and `char_to_text_length` decided for each other's family of types, so configuring one of the two turned the whole other family into `TEXT`.

  - Fix - The schema of the migrator metadata cannot be `public` or empty any more: it is dropped with everything in it at the start of every run.

  - Fix - Sequences: the `mapping` workflow set them from the data in the target alone, the protocol reported the position of a sequence as the value it starts at, and the base type of a Sybase ASE user defined type was resolved by a join on the type code, which several system types share.

  - Fix - Sybase ASE: a procedure which selects rows and returns a status code at the same time was converted into a function PostgreSQL refuses to create, and the conversion of a routine ended with a parser failure whenever the `target` section carried a `connectivity` key.

- 2026.08.19

  - New - The `tests/` directory is published with the repository. It was excluded from git, so nothing in it was ever installed or run by anybody else.

  - Fix - A configuration file without a `migration:` block died with `KeyError` on the first thing it was asked.

  - New - Every table the migrator keeps in its own database carries its description in the database, as a comment.

- 2026.08.18

  - New - SQL Anywhere: the procedures, the functions, the triggers and the sequences of the source are migrated. They were read and never converted, and `fetch_sequences` answered nothing at all.

  - Fix - SQL Anywhere: the foreign keys were read from the wrong side of the catalogue, so every one of them pointed the wrong way.

  - Fix - Only `CASCADE` was written into a migrated foreign key: a source reporting `SET NULL`, `SET DEFAULT` or `RESTRICT` lost its rule.

  - New - Sybase ASE: procedure groups are migrated - a set of procedures sharing one name, numbered, which PostgreSQL has no counterpart for and which are created as separate routines.

  - Fix - Sybase ASE and the T-SQL parser: a round of repairs to the conversion of routines and triggers - a cursor declaration cut in front of its own query, `SET ROWCOUNT` with a variable, a body ending in a comment, an output parameter next to a default, `@@rowcount` outside a condition, an assignment reading from a table, a type name mapped twice, the message catalogue of `raiserror`, and a table with a `timestamp` column whose data the target refused.

  - Fix - SQLite: a CHECK constraint against a flag column was refused by the target, the data of a whole table was rejected when one column held a value of another storage class, and a `BLOB` migrated into a text column was decoded with replacement characters.

  - Fix - The three Db2 connectors: a trigger calling one of the casting scalar functions of Db2 could not be created.

- 2026.08.17

  - Fix - Db2 for i and Db2 for z/OS: a date, a time or a timestamp of a CSV export reached the target in the format of the export instead of as a value, and a date written with a two digit year does not say which of its parts is the day - the migrator no longer guesses.

- 2026.08.13

  - Fix - Anonymization: a method name which is not registered stops the run before any data is read, and a value which does not fit into the length of the target column is no longer cut without a word.

- 2026.08.12

  - Fix - Informix: a view with an outer join was never created - Informix marks the subordinate table of an outer join in a way no parser reads.

  - Fix - Sybase ASE and the T-SQL parser: the pseudo tables `inserted` and `deleted` of a trigger are converted statement by statement, a cursor inside a trigger is no longer cut at the `SELECT` of its query, the global variables of Sybase are converted, `SET @variable = value` and `SELECT @a = 1, @b = 2` are read, a `WHILE` written against its keyword is recognised, `ELSE BEGIN` on one line is read as both, an `INSERT ... SELECT` split by an empty line is no longer cut in two, and a block comment containing another one no longer ends at the first `*/`.

  - Fix - A function of the source which the mapping describes as a call was not found when the code wrote a space in front of the parenthesis.

- 2026.08.11

  - Fix - Sybase ASE: a trigger was migrated for every table it reads or writes instead of the table it is declared on; a routine written as `CREATE OR REPLACE PROCEDURE` was created with that text in its body; every table CHECK constraint was migrated twice; and a rule of the source could not be created as a domain.

  - Fix - MS SQL Server: no statement of a routine was converted at all - the parser was built without the conversions - and `user_name()` and `suser_sname()` became a function PostgreSQL does not have.

  - Fix - The T-SQL parser: only the `SELECT` statements of a routine were converted, `DELETE FROM t FROM t, x` was not read, a `CASE` written over several lines was torn apart, and a comment containing the word `AS` ended the header of the routine it stood in front of.

  - Fix - Sequences: the next identity value of the source was applied to every sequence a table owns rather than to the one which belongs to the column, a sequence could be set below the data it belongs to, and the sequence of an identity column was set only for a table which received rows in that run.

  - Fix - A foreign key whose column has another data type than the column it references is aligned instead of failing.

- 2026.08.10

  - Fix - **Informix: the whole system catalogue was migrated together with the user data model** - 534 routines and every system table among them.

  - Fix - Informix: a large round of repairs to everything the connector reads. Tables with `BSON`/`JSON`, collection and row type columns, descending and function based indexes, `INTERVAL` and `MONEY` columns, the constraints, the triggers - none of the eight triggers of the test model could be created - the routines with their data types, their global variables and their transaction control, the `MATCHES` operator, and the SQL functions inside a routine, a view or a check constraint, which were never converted at all.

  - Fix - JDBC connectors: the migrator printed its summary and then hung forever instead of ending.

- 2026.08.07

  - Fix - Informix: default values were read in a way which both corrupted the value and dropped it, `BSON` and `JSON` columns were not converted, and the code of a routine created as `CREATE DBA PROCEDURE` was not read.

  - Fix - A data type without a mapping to the target was passed into the DDL unchanged and without a word, and a NUL character anywhere in the metadata read from a source aborted the migration phase.

- 2026.08.06

  - New - **PostgreSQL as a source: collations and full text search objects are migrated** - user defined collations, text search dictionaries and configurations - and the references to them inside views, indexes and generated columns are qualified so that they resolve against the migrated schema.

  - New - User defined aggregates are migrated, together with the functions and procedures and always behind them.

  - New - The pre-migration analysis checks the PostgreSQL extensions: every extension of the source with its version, whether the target has it, and whether everything the migrated objects depend on is covered.

  - Fix - PostgreSQL as a source: generated columns were silently degraded to ordinary columns, indexes lost everything their definition carries behind the key list (`INCLUDE`, `NULLS NOT DISTINCT`, storage parameters, the `WHERE` of a partial index), a plain unique index lost its uniqueness, functional indexes were truncated at the first closing parenthesis, a `CREATE CONSTRAINT TRIGGER` was migrated as an `ALTER TABLE`, and column collations were not read at all.

  - Fix - The data copy of the anonymization workflow and the LOB import built their `INSERT` from all target columns rather than from the ones they had values for.

- 2026.08.05

  - Fix - A round of repairs to the PostgreSQL to PostgreSQL migration of both schema and data.

- 2026.08.04

  - New - **New source connector: SQLite**, with `native` connectivity for a database file and `ddl` connectivity for SQL script files with the data delivered as CSV.

  - Fix - Db2 for i: a large round - the connector could not be instantiated at all, tables were created without any columns, primary keys, foreign keys, unique and check constraints were lost entirely, tables were skipped in silence, views could not be created, materialized query tables were parsed as ordinary views, expression indexes and triggers failed, and the data migration from the unload files did not start.

  - New - Db2 for z/OS: functions, procedures and global variables are migrated, and the constraints, the views, the triggers and the expression indexes were repaired in the same round.

  - Fix - The data source file of a table was not found when the table has more than one alias, or when the name of the file does not have the case the placeholder produced.

  - Fix - The comments migration crashed on a variable which had not been set.

- 2026.08.03

  - New - **New source connector: IBM Db2 for i**, migrating the structure from DDL script files and the data from unload files.

  - Fix - Db2 LUW: views could not be created for a long list of reasons - unqualified names, recursive CTEs, `LISTAGG`, materialized query tables, `SIGNAL SQLSTATE` inside triggers and routines, composite key columns parsed as one name, and `names_case_handling` which the connector applied inconsistently.

- 2026.07.31

  - Fix - Db2 LUW: table creation failed on a cast between two time types, and function based indexes were created against a column which does not exist.

- 2026.07.30

  - Fix - MS SQL Server: a round of repairs - table creation, a connection busy with the results of another command, `datetimeoffset`, `ROWVERSION` and `XML` columns, the headers of procedures and functions, and indexes on a type which has no default operator class.

  - New - A `UNIQUE` constraint is generated automatically on the parent side of a foreign key which references a column set the source did not declare unique.

  - Fix - SQL Anywhere: the type mapping, the view conversion and the function mapping were filled in - they were nearly empty.

- 2026.07.29

  - Fix - MySQL and MariaDB: a round of repairs - sequences and column defaults, views using inline `IF(...)`, `array.array` values, indexes with an empty column list, spatial columns with no default operator class, and the string formatting of an `INSERT`.

  - New - Configurable handling of the all-zero date, datetime and timestamp of MySQL, both as a default and as data, with an option to relax the `NOT NULL` which goes with it.

  - New - `migration.uuid_default_function` decides which function a UUID default becomes, and `CAST(expr AS CHAR(N))` / `CAST(expr AS UNSIGNED)` are transpiled for every source.

- 2026.07.27

  - New - **Oracle: packages are migrated as standalone functions** instead of being skipped - PostgreSQL has no packages, and `migration.packages_as` decides between one function per routine and a schema per package.

  - New - The pre-migration analysis ends with a target capability check: what the source schema needs and the target cannot express stops the run before anything is created.

  - New - Oracle: virtual columns are migrated as PostgreSQL generated columns, and function based indexes are created as expression indexes.

  - Fix - Oracle: the references to the source schema inside a converted routine are re-pointed at the target, invisible columns no longer poison the column list, object type columns are no longer mistaken for virtual columns, the column defaults are translated instead of being passed through, the system managed pseudo tables are no longer listed for migration, and a failing table description no longer aborts the table.

- 2026.07.23

  - New - **A final object-validity pass** at the end of a standard migration: an object whose creation failed only because a dependency did not exist yet is retried once the whole schema is there.

  - New - Oracle: the data of user defined type columns is migrated, `SDO_GEOMETRY` is migrated best-effort without PostGIS, `XMLTYPE` values are materialized before insert, and `INTERVAL YEAR TO MONTH` is written as a PostgreSQL interval.

  - New - A narrow numeric source column - Oracle `NUMBER(1,0)` and its equivalents - can be mapped to `BOOLEAN` by configuration; it stays `SMALLINT` by default, so `WHERE FLAG = 1` keeps working.

- 2026.07.22

  - New - **Oracle: a large round of new objects.** Sequences as standalone objects, user defined types and domains, CHECK constraints, triggers and routines converted best-effort from PL/SQL to PL/pgSQL, views and materialized views with a real Oracle to PostgreSQL query conversion, and the table and column comments.

  - Fix - Oracle: 12c identity columns are detected through the catalogue rather than through their default expression, `RAW` and `LONG RAW` are no longer classified as string types, and `connect()` reports the error it really got instead of "oracledb module is not installed".

- 2026.07.07

  - Fix - Validation: optional configuration blocks written as null, the column key of the mapping workflow, the precision of approximate data types, the table iteration of the mapping workflow, and the target copy connection.

- 2026.07.03

  - New - Validation: the engine differentiates between a standard migration and the `mapping` workflow, and it reads the `data_conflict_action` the migration really used before it compares anything.

  - New - Reporting: the `[ DATABASE CONTEXT ]` block of the validation summary, and a redesigned `mapping_report.md` with a configuration block and aggregated per-table statistics.

- 2026.07.02

  - Fix - The target schema was dropped unconditionally during pre-planning, whatever `migration.drop_schema` said.
