...
- 2026.06.09
  ...
  - Fix - Validation: Excluded Primary Keys (`p`) and Unique Constraints (`u`) from the constraint-level validation block because they are exclusively mapped and handled inside the Indexes validation block (aligning with Oracle's structure mapping behavior). This prevents phantom, unmatched duplicate constraints from cluttering the final validation report.
