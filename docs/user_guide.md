...
- **Partitioning** has two halves which are worth telling apart. *Reading how the source partitions* is per connector and is implemented for **every one of the twelve** whose engine has partitioning at all; SQL Anywhere and SQLite have none, and say so. Where an engine calls something "partitioning" which PostgreSQL has no counterpart for — Oracle's REFERENCE and SYSTEM schemes, Db2's DPF and MDC, Informix's ROUND ROBIN fragmentation, SQL Server's filegroups — it is named per table rather than quietly dropped, and a scheme which cannot be built stops the run before anything is created. *Creating the target partitioned* — the `target_partitioning` block — reads a configuration and writes PostgreSQL, looks at nothing of the source, and therefore works for every connector. Both are reported in the `[ PARTITIONING ]` block of the pre-migration analysis before anything is created. **[Partitioning](partitioning.md) has the whole of it, per source database**, including what each engine's bounds become and why five of the twelve need every bound converted rather than copied.
