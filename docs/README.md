...
- **Migrated**: tables (including **partitioned tables**, on which see further down this bullet), data, primary keys, indexes, foreign keys with their `ON DELETE`/`ON UPDATE` rules, CHECK constraints, identity columns, generated columns, collations, full text search dictionaries and configurations, domains, user-defined types (enums, composite types, range types), sequences (`CREATE SEQUENCE` + `setval`, continuing from the source position), views and materialized views, functions, procedures and aggregates, triggers, and table/column comments. **Partitioning** is read from the source, and the migration says per table what it does with it: `migration.source_partitioning: preserve` (the default) recreates the scheme — the parent with its `PARTITION BY` clause, every partition as `PARTITION OF … FOR VALUES …`, sub-partitions and all — and the rows go in through the parent, so PostgreSQL routes each one to where it belongs; `flatten` builds one ordinary table and says so. It is settable per table in `table_settings`, and `target_partitioning` — a scheme the source never had — wins over both. A **partition is never migrated as a table of its own** under any of the three: it is created with its parent, which is what keeps its rows from being written twice. The pre-migration analysis reports the scheme of every partitioned table and **stops the run** for what cannot be built — a method the target version does not have, or a primary key which does not contain the partitioning columns, which PostgreSQL refuses on a partitioned table. `--convert-queries` converts the statements of an application here too, although the statement is already the dialect of the target: what it adds is the map of the migration, the test against the new database, and the constructs a migration takes away — a statement reading a system column (`ctid`, `xmin`, `tableoid`) is refused, because the migration inserted the rows again in an order of its own and every one of those values is another one on the target, while the target accepts the statement without a word.
