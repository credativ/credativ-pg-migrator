...
def convert_view(connector, body):
...
def test_star_equals_becomes_a_left_join(name, build):
    answer = convert_query(build(), 'SELECT c.id, o.total FROM dbo.customer c, dbo.orders o '
                                    'WHERE c.id *= o.cid')
    assert answer['converted'] is True, answer['error']
    assert 'LEFT JOIN' in answer['code']
    assert 'ON "c"."id" = "o"."cid"' in answer['code']
...
def test_equals_star_keeps_the_rows_of_the_table_the_asterisk_names(name, build):
    ...
    answer = convert_query(build(), 'SELECT c.id FROM dbo.customer c, dbo.orders o '
                                    'WHERE c.id =* o.cid')
    assert answer['converted'] is True, answer['error']
    assert 'RIGHT JOIN "tgt"."orders"' in answer['code']
...
def test_the_operator_never_survives_the_conversion(name, build):
    ...
    assert 'right_outer' not in answer['code']


@BOTH
def test_two_outer_joins_in_one_statement(name, build):
    answer = convert_query(build(), 'SELECT c.id FROM dbo.customer c, dbo.orders o, dbo.items i '
                                    'WHERE c.id *= o.cid AND o.id *= i.oid')
    assert answer['converted'] is True, answer['error']
    assert answer['code'].count('LEFT JOIN') == 2
...
def test_the_true_left_where_the_condition_stood_is_taken_out(name, build):
    ...
    assert 'TRUE' not in answer['code'].upper()
...
def test_a_condition_on_the_inner_table_moves_into_the_on_clause(name, build):
    ...
    assert answer['converted'] is True, answer['error']
    assert 'ON "c"."id" = "o"."cid" AND "o"."status" = \'X\'' in answer['code']
    assert 'WHERE' not in answer['code']
...
def test_a_test_for_null_on_the_inner_table_does_not_move(name, build):
    ...
    assert answer['converted'] is True, answer['error']
    assert 'WHERE "o"."cid" IS NULL' in answer['code']
...
def test_a_condition_reading_two_tables_does_not_move(name, build):
    ...
    assert '"o"."total" > "c"."credit_limit"' in answer['code'].split('WHERE')[1]
...
def test_a_parenthesised_or_on_the_inner_table_keeps_its_parentheses(name, build):
    ...
    assert answer['converted'] is True, answer['error']
    assert "AND (\"o\".\"status\" = 'X' OR \"o\".\"status\" = 'Y')" in answer['code']
...
def test_an_or_left_in_the_where_clause_keeps_its_parentheses_too(name, build):
    ...
    assert answer['converted'] is True, answer['error']
    assert 'WHERE ("c"."a" = 1 OR "c"."b" = 2)' in answer['code']
...
def test_a_join_written_as_ansi_in_the_source_is_left_alone(name, build):
    ...
    """
    answer = convert_query(build(), "SELECT c.id FROM dbo.customer c "
                                    "LEFT JOIN dbo.orders o ON c.id = o.cid WHERE o.status = 'X'")
    assert answer['converted'] is True, answer['error']
    assert 'WHERE "o"."status" = \'X\'' in answer['code']


@BOTH
def test_an_ansi_join_beside_a_legacy_one_keeps_its_own_where_clause(name, build):
    answer = convert_query(build(),
                           "SELECT c.id FROM dbo.customer c, dbo.orders o "
                           "LEFT JOIN dbo.items i ON o.id = i.oid "
    ...
    assert '"o"."status" = \'X\'' in answer['code'].split('WHERE')[0]
    assert '"i"."kind" = \'K\'' in answer['code'].split('WHERE')[1]
...
def test_a_statement_without_an_outer_join_is_not_touched(name, build):
    answer = convert_query(build(), "SELECT c.id FROM dbo.customer c, dbo.orders o "
                                    "WHERE c.id = o.cid AND o.status = 'X'")
    assert answer['converted'] is True, answer['error']
    assert 'JOIN' not in answer['code']
    assert '"o"."status" = \'X\'' in answer['code'].split('WHERE')[1]
...
def test_the_view_path_converts_the_outer_join_too(name, build):
    ddl = convert_view(build(), 'SELECT c.id, o.total FROM dbo.customer c, dbo.orders o '
                                'WHERE c.id *= o.cid')
...
def test_the_view_path_moves_the_condition_of_the_inner_table_as_well(name, build):
    ddl = convert_view(build(), "SELECT c.id FROM dbo.customer c, dbo.orders o "
                                "WHERE c.id *= o.cid AND o.status = 'X'")
...
def test_a_view_never_carries_a_marker_into_the_target(name, build):
    ...
    fewer rows than the view of the source did.
    """
    ddl = convert_view(build(), 'SELECT c.id FROM dbo.customer c, dbo.orders o '
                                'WHERE c.id *= o.cid OR c.id = 1')
    assert 'left_outer' not in ddl
    assert 'right_outer' not in ddl
    assert not ddl.strip().upper().startswith('CREATE OR REPLACE VIEW "TGT"."V_TEST" AS\nSELECT "C"."ID" FROM "TGT"."CUSTOMER" AS "C", ')


## ---------------------------------------------------------------- the gates see it as a read

@pytest.mark.parametrize('source_db_type, name, build',
                         [('mssql',) + case for case in CONNECTORS[:1]]
                         + [('sybase_ase',) + case for case in CONNECTORS[1:]],
                         ids=['ms_sql', 'sybase_ase'])
def test_a_statement_with_an_outer_join_is_classified_as_a_read(source_db_type, name, build):
    """
