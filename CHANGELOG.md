...
- 2026.07.29
  ...
  - Feature - Connectors / Base Database Connector: Added automatic conversion of MySQL/MariaDB `GROUP BY <exprs> WITH ROLLUP` syntax to ANSI/PostgreSQL standard `GROUP BY ROLLUP (<exprs>)`. Also added automatic stripping of character set and collation specifications (`CHARACTER SET <charset>`, `CHARSET <charset>`, `COLLATE <collation>`), character set introducers (`_utf8mb4'...'`, `_latin1"..."`), and backslash quote escapes (`\'`, `\"`) when converting SQL code, views, functions, and expression indexes for target PostgreSQL. Transpiles MySQL multi-valued/JSON index expressions (`CAST(... AS CHAR(32) ARRAY)`) to valid PostgreSQL dialect (`CAST(... AS CHAR(32)[])`). Prevents `syntax error at or near "WITH"`, `syntax error at or near "character"`, and `syntax error at or near "\"` errors during PostgreSQL view and expression index creation.
